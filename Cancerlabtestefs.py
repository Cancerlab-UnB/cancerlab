...
def data_uri_cached(relpath: str, mime: str | None = None) -> str:
    """Return a data: URI for the image, cached and auto-busted when the file changes."""
    p = (Path(__file__).parent / relpath).resolve()
    if not p.exists():
        raise FileNotFoundError(f"Asset not found: {relpath}")
    # include the file's mtime in the cache key so edits bust the cache automatically
...
st.set_page_config(
    ...
    page_icon=str(ICON_PATH),  # em vez de Image.open(...)
    layout="wide"
)
...
def data_uri_cached(relpath: str, mime: str | None = None) -> str:
    ...
    mime = mime or (mimetypes.guess_type(str(p))[0] or "image/jpeg")
    ...
    if unbimg is None:
        try:
            ...
            for pth, mime in candidates:
                if Path(pth).exists():
                    unbimg = data_uri_cached(pth, mime=mime)
                    break
        except Exception:
            unbimg = ""
...
<style>
  :root {{
    --anni-accent: var(--bar-bg, #3f5eb5);  /* your site blue */
    --anni-ink: #0f172a;
    --anni-line: #e6eaf2;
  }}
  .{uid}-wrap {{
  ...
  .{uid}-pill, .{uid}-pill * {{ color:#fff !important; -webkit-text-fill-color:#fff; }}
  ...
  .{uid}-backdrop {{
    position:absolute; inset:0; background:rgba(2,6,23,.28);
    backdrop-filter: blur(2px); -webkit-backdrop-filter: blur(2px);
  }}
  .{uid}-fx {{ position:absolute; inset:0; pointer-events:none; overflow:hidden; }}
  .{uid}-balloon {{
    position:absolute; bottom:-18vh; width:54px; height:68px;
    border-radius:50% 50% 46% 46%;
    background: radial-gradient(circle at 35% 28%, rgba(255,255,255,.9), rgba(255,255,255,.75) 40%, rgba(255,255,255,.6) 70%);
    border:1px solid rgba(15,23,42,.08);
    box-shadow:0 6px 12px rgba(2,6,23,.12);
    animation: {uid}-float var(--spd, 12s) linear infinite;
    opacity:.95;
  }}
  .{uid}-balloon::after {{
    content:""; position:absolute; left:50%; bottom:-24px; width:1.5px; height:28px;
    background:rgba(15,23,42,.25); transform:translateX(-50%);
  }}
  @keyframes {uid}-float {{
    0%   {{ transform: translateY(0) translateX(0); }}
    50%  {{ transform: translateY(-44vh) translateX(8px); }}
    100% {{ transform: translateY(-88vh) translateX(-6px); }}
  }}
  .{uid}-card {{
    position:relative; max-width:min(92vw, 680px);
    background:#ffffff; color:var(--anni-ink);
...
<div class="{uid}-wrap">
  ...
  <div class="{uid}-modal" role="dialog" aria-modal="true">
    ...
    <!-- 6 subtle white balloons -->
    <div class="{uid}-fx">
      <span class="{uid}-balloon" style="left:10%; --spd:12.5s;"></span>
      <span class="{uid}-balloon" style="left:24%; --spd:11.8s;"></span>
      <span class="{uid}-balloon" style="left:38%; --spd:12.2s;"></span>
      <span class="{uid}-balloon" style="left:52%; --spd:11.5s;"></span>
      <span class="{uid}-balloon" style="left:66%; --spd:12.6s;"></span>
      <span class="{uid}-balloon" style="left:80%; --spd:11.9s;"></span>
    </div>
    <div class="{uid}-card">
...
# img sem alterações
    left_img = data_uri_cached("imghome1.jpg", mime="image/jpeg")
    right_img = data_uri_cached("imghome2.jpg", mime="image/jpeg")
    landmarks_img = data_uri_cached("landmarks.jpg", mime="image/jpeg")
...
def page_positions():
    ...
    def _video_data_uri(p: Path | None) -> str | None:
        ...
        try:
            return data_uri_cached(str(p), mime=_mime_for(p)) if p else None
        except Exception:
            return None
...
# IMPORTANTE: pressupondo que 'engine' e 'metadata' já estão definidos acima.
# Ex.: engine = get_engine() com @st.cache_resource

def _bootstrap_schema_once():
    # checkfirst=True é o padrão, mas deixamos explícito
    metadata.create_all(engine, checkfirst=True)
    return True

# Chame isso uma vez no início do app (fora de handlers que rodam muitas vezes)
_ = _bootstrap_schema_once()
