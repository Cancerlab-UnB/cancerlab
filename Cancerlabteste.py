...
def safe_dataframe(df, *args, **kwargs):
    try:
        if isinstance(df, pd.DataFrame) and len(df) > MAX_ROWS_GLOBAL:
            st.caption(f"⚠️ Mostrando apenas {MAX_ROWS_GLOBAL} de {len(df)} linhas (modo seguro)")
...
# PÁGINA INICIAL
    ...
    def painel_agenda_bxl(file_path="Entrada e Acompanhamento de Pacientes Biópsia Líquida CCR.xlsx"):
        ...
        - WhatsApp: apenas coletas de hoje até +30 dias
        ...
        def painel_eventos(df_pat):
            ...
            st.dataframe(
                df_evt[["CBL", "EventType", "EventDate_str", "Segmento", "CBL_RAW"]].rename(columns={
        ...
        def painel_whatsapp_bxl(df_all_events, phones_map, headers, phone_cols):
            ...
            st.caption("Apenas coletas futuras entre hoje e +30 dias.")
            ...
            ev["Label"] = ev.apply(
                lambda r: f"{str(r['Coleta_short']).strip()} — {str(r['Data_str']).strip()} (D{_safe_int(r['Dias_num'])})",
                axis=1
            )
        ...
        SHOW_COLS = ["CBL", "Coleta_show", "Data_str", "Dias_num", "Status_display", "Cell"]
        ...
        # WHATSAPP
        # =========================================================
        painel_whatsapp_bxl(df_all, phones_map, headers, phone_cols)
