...
# PÁGINA INICIAL
    ...
    def painel_agenda_bxl(file_path="Entrada e Acompanhamento de Pacientes Biópsia Líquida CCR.xlsx"):
        """
        PAINEL AGENDA BxL — robusto para a planilha atual
    
        REGRAS:
        - Toda coleta com data válida e SEM "(coletado)" entra como pendente
        - Se Dias < 0 => ATRASADA
        - Se Dias >= 0 => FUTURA/PENDENTE
        - Pacientes com ID contendo:
            * "exc"   -> evento
            * "saiu"  -> evento
            * "+"     -> falecimento
          não aparecem na agenda / WhatsApp
        - WhatsApp apenas para coletas 0..30 dias, sem atrasadas
        ...
        def _find_main_columns(headers):
            ...
            for idx, h in enumerate(headers, start=1):
                ...
                if cols["id"] is None and ("cbl" in nh or ("id" in nh and "paciente" in nh)):
                    cols["id"] = idx
        ...
        def _clean_date_text(s):
            ...
            # normaliza separadores e lixo comum
            s = s.replace("\\", "/")
            s = s.replace("–", "-").replace("—", "-")
            s = re.sub(r"[;,]+", " ", s)
            s = re.sub(r"\s+", " ", s).strip()
    
            # corrige barras e hífens duplicados
            s = re.sub(r"/{2,}", "/", s)
            s = re.sub(r"-{2,}", "-", s)
            s = re.sub(r"\.{2,}", ".", s)
    
            # corrige formatos como 04//07/24
        ...
        def _extract_first_date_from_string(s, today_):
            ...
            # prioridade: dd/mm/yyyy ou dd/mm/yy
            m = RE_DMY.search(s0)
            if m:
                d, mo, y = m.groups()
                try:
        ...
        def _apply_cell_mark(ws, cell_ref, d, status, motivo, obs):
            ...
            txt = f"{stt if stt else ('ATRASADA' if is_late else 'REGISTRADA')} | {now}\n{motivo}".strip()
            if obs:
                txt += f"\nObs: {str(obs).strip()}"
        ...
        def _load_events_and_patients(sheet_name):
            ...
            bxl_cols = _find_bxl_columns(headers)
            ...
            df_pat = pd.DataFrame(event_rows).drop_duplicates(subset=["CBL", "CBL_RAW"])
            ...
            # segurança extra: coletado nunca entra
        ...
        def painel_eventos(df_pat):
            ...
            vc = df_evt["EventType"].replace("", "EVENTO NÃO ESPECIFICADO").fillna("EVENTO NÃO ESPECIFICADO").value_counts().reset_index()
        ...
        def painel_whatsapp_bxl(df_all_events, phones_map, headers, phone_cols):
            ...
            # regra do WhatsApp: nunca registradas, nunca coletadas, nunca atrasadas, apenas 0..30
            df_src = df[
        ...
        hide_withdrawn = st.sidebar.checkbox("Ocultar retirados do estudo", value=True, key="bxl_hide_withdrawn")
        ...
        pend["Dias_num"] = pd.to_numeric(pend["Dias"], errors="coerce")
    
        atras = pend[pend["Dias_num"] < 0].copy()
        atras = atras.sort_values(["Data", "CBL", "Coleta_short"]).reset_index(drop=True)
    
        fut = pend[pend["Dias_num"] >= 0].copy()
        fut = fut.sort_values(["Data", "CBL", "Coleta_short"]).reset_index(drop=True)
        ...
        SHOW_COLS = ["CBL", "Coleta_show", "Data_str", "Dias_num", "Status_display"]
        ...
        with tabP:
            if pend.empty:
                ...
                st.dataframe(
                    pend[SHOW_COLS].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status", "Dias_num": "Dias"}),
                    use_container_width=True,
                    hide_index=True,
                    height=420
                )
        ...
        with tabA:
            if atras.empty:
                ...
                st.dataframe(
                    atras[SHOW_COLS].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status", "Dias_num": "Dias"}),
                    use_container_width=True,
                    hide_index=True,
                    height=420
                )
        ...
        with tab1:
            if fut_0_7.empty:
                ...
                st.dataframe(
                    fut_0_7[SHOW_COLS].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status", "Dias_num": "Dias"}),
                    use_container_width=True,
                    hide_index=True,
                    height=420
                )
        ...
        with tab2:
            if fut_8_14.empty:
                ...
                st.dataframe(
                    fut_8_14[SHOW_COLS].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status", "Dias_num": "Dias"}),
                    use_container_width=True,
                    hide_index=True,
                    height=420
                )
        ...
        with tab3:
            if fut_15_30.empty:
                ...
                st.dataframe(
                    fut_15_30[SHOW_COLS].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status", "Dias_num": "Dias"}),
                    use_container_width=True,
                    hide_index=True,
                    height=420
                )
        ...
        with tab4:
            if fut_31mais.empty:
                ...
                st.dataframe(
                    fut_31mais[SHOW_COLS].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status", "Dias_num": "Dias"}),
                    use_container_width=True,
                    hide_index=True,
                    height=420
                )
        ...
        auto = df_all[
            (pd.to_numeric(df_all["Dias"], errors="coerce") < 0) &
            (~df_all["CollectedTag"].astype(bool)) &
            (df_all["Status"].astype(str).str.upper() != "REGISTRADA")
        ].copy()
            ...
            for _, r in auto.iterrows():
                try:
                    _save_status_to_excel(
                        ...
                        dd=r["Data"],
                        status="ATRASADA",
                        motivo="Auto: coleta vencida e não registrada como coletada",
                        obs="",
        ...
        if action_df.empty:
            ...
            with b3:
                st.caption(
                    f"CBL: {sel_row['CBL']} | Coleta: {sel_row['Coleta_short']} | Data: {sel_row['Data_str']} | Dias: {_safe_int(sel_row['Dias_num'])} | Status: {sel_row['Status_display']}"
                )
    ...
    def _cell_style(val):
        ...
        if d and d.date() < datetime.now().date() and "(coletado)" not in raw:
    ...
    styler = df_show.style.apply(_highlight_excluidos, axis=1).applymap(_cell_style)
