...
# PÁGINA INICIAL
    ...
    def painel_biopsia(df: pd.DataFrame):
        ...
        def _is_date_col(s: pd.Series) -> bool:
            if pd.api.types.is_datetime64_any_dtype(s):
                return True
            # também evita colunas que são strings mas claramente datas
            name = str(s.name).lower()
            if "data" in name:
                return True
            return False
        
        def _to_categoria_com_faixas(s: pd.Series, max_bins: int = 8) -> pd.Series:
            """
            Converte uma série em categórica:
            - se for numérica com poucos valores únicos → categorias diretas
            - se for numérica com muitos valores → faixas (bins)
            - se for não numérica → texto
            """
            s_orig = s.copy()
        
            # tentar converter numérica
            s_num = pd.to_numeric(s_orig, errors="coerce")
        
            # se quase tudo vira NaN, trata como texto
            if s_num.notna().sum() < 0.3 * len(s_orig):
                return s_orig.astype(str).replace("", np.nan)
        
            # é numérica de verdade
            n_unique = s_num.dropna().nunique()
        
            # poucos valores únicos → trata cada valor como categoria
            if n_unique <= max_bins:
                return s_num.astype(str)
        
            # muitos valores → criar faixas (quantis)
            try:
                bins = min(max_bins, n_unique)
                cuts = pd.qcut(s_num, q=bins, duplicates="drop")
                out = pd.Series(index=s_orig.index, dtype="object")
                out.loc[s_num.index] = cuts.astype(str)
                return out
            except Exception:
                # fallback: converte para string mesmo
                return s_num.astype(str)
        
        # ---- DISTRIBUIÇÕES / CORRELAÇÕES SIMPLES
        with tab_dist:
            # trabalhamos numa cópia para criar variáveis derivadas
            dist_df = df.copy()
        
            # --------- criar variáveis derivadas de BxL (Fez / Não fez) ---------
            for c in dist_df.columns:
                cname = str(c).lower()
                if "bxl" in cname:  # qualquer coluna que tenha "bxl" no nome
                    s = dist_df[c]
                    has = s.notna() & (s.astype(str).str.strip() != "")
                    novo_nome = f"Fez {c}"
                    dist_df[novo_nome] = np.where(has, "Sim", "Não")
        
            # --------- escolher colunas candidatas (sem datas) ---------
            candidate_cols = []
            for c in dist_df.columns:
                s = dist_df[c]
                if s.dropna().empty:
            ...
            if not candidate_cols:
                st.info("Não encontrei colunas elegíveis para distribuição/correlação.")
            else:
                col_x = st.selectbox("Variável principal", candidate_cols, index=0)
                col_y = st.selectbox(
                    "Correlacionar com",
                    ["(nenhuma)"] + candidate_cols,
                    index=0
                )
        
                # ==================== CASO 1: VARIÁVEL ISOLADA ====================
                if col_y == "(nenhuma)":
                    s1 = dist_df[col_x]
        
                    # converte para categorias (inclui faixas se for numérico grande)
                    cat1 = _to_categoria_com_faixas(s1)
                    ...
                    fig.update_layout(xaxis_title=col_x, yaxis_title="Número de pacientes")
                    fig = _apply_fig_base(fig, h=420)
                    st.plotly_chart(fig, use_container_width=True)
        
                # ==================== CASO 2: CORRELAÇÃO ENTRE 2 VARIÁVEIS ====================
                else:
                    if col_y == col_x:
                        st.warning("Selecione uma segunda variável diferente para correlação.")
                    else:
                        s1 = dist_df[col_x]
                        s2 = dist_df[col_y]
        
                        # transforma as duas em categóricas (criando faixas se forem numéricas grandes)
                        cat1 = _to_categoria_com_faixas(s1)
                        cat2 = _to_categoria_com_faixas(s2)
                        ...
                        if t.empty:
                            st.info("Não há dados suficientes para essa combinação de variáveis.")
                        else:
                            t_counts = (
                                t.value_counts()
        ...
        # ---- TENDÊNCIAS (robusto: total, por Coleta, por variável clínica ou painel IA)
        with tab_trend:
            if cal_long.empty:
                ...
                # ================== 3) POR VARIÁVEL CLÍNICA (CATEGÓRICA) ==================
                elif modo == "Por variável clínica (categórica)":
                    candidates = []
                    for k in [
                        "sexo", "estadiamento", "tabagismo_status",
                        "hist_fam_1g", "hist_fam_2g", "ecog", "has", "dm",
                    ...
                    if not candidates:
                        ...
                        if C.get("CBL") and (C["CBL"] in dd.columns or C["CBL"] in df.columns):
    ...
    if "STATUS ESTUDO" in df_visu.columns:
        st.dataframe(
            df_visu.style.apply(_highlight_excluidos, axis=1),
            use_container_width=True
        )
    else:
    ...
    if st.session_state.get(flow_key, False):
        ...
        # usa a 1ª coluna como identificador (CBL / ID do paciente)
        ...
        with col_ok:
            if st.button("✅ Confirmar retirada do estudo", key=f"btn_confirmar_exclusao_{idx}"):
                if not confirm_flag:
                    ...
                    # preenche apenas campos vazios com o texto de exclusão
                    # preenche apenas campos vazios com o texto de exclusão,
                    # mas NÃO mexe nas colunas de arquivo/data
                    COLUNAS_PROTEGIDAS = {
                        "TCLE FOTO",
                        "TCLE",
                        "data TCLE",
                    ...
                    # marca o identificador com ícone de excluído (se ainda não tiver)
    ...
    # função auxiliar: só considera válido se for caminho de imagem existente
    ...
    if _tcle_valido(tcle_path):
        ...
        # Não há TCLE válido (célula vazia, texto qualquer ou caminho quebrado)
