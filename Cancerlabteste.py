...
# PÁGINA INICIAL
    ...
    # =====================================================================
    # 2) PAINEL AGENDA BxL (Entrada/Acompanhamento) — buckets + WhatsApp
    # =====================================================================
    def painel_agenda_bxl(file_path: str = "Entrada e Acompanhamento de Pacientes Biópsia Líquida CCR.xlsx"):
        ...
        # ============================================================
        # Datas — extrair TODAS as datas possíveis de uma célula
        # ============================================================
        ...
        def _parse_token_to_date(token: str, today: date) -> date | None:
            ...
            # 3) dd/mm (sem ano) — assume ano atual; se já passou, assume próximo ano
        ...
        def _preprocess_month_text(s: str) -> str:
            ...
            para algo numérico parseável.
            """
            s2 = " " + s.lower() + " "
            s2 = s2.replace("º", "").replace("ª", "")
            s2 = re.sub(r"[,]+", " ", s2)
            # substitui meses por número
        ...
        def _extract_all_dates(x, today: date, epoch) -> list[date]:
            """
            Retorna lista de dates encontradas na célula:
            - datetime/date: 1 data
            - serial Excel: 1 data
            - strings: TODAS as datas encontradas no texto (inclusive múltiplas)
            """
            out: list[date] = []
            ...
            if isinstance(x, (int, float, np.integer, np.floating)):
                # proteção: só tenta serial em faixa plausível
                try:
                    v = float(x)
                    if -100000 < v < 500000:
            ...
            # string: extrair todas as datas possíveis
            s = str(x).strip()
            if not s or s.lower() in ("nan", "na", "n/a", "-"):
                return out
            ...
            # pega dd/mm sem ano (mas evita capturar partes já capturadas por DMY/YMD)
            # estratégia: só pega DM se não estiver colado a um ano ali perto
            dm_candidates = _RE_DM.findall(s2)
            for dm in dm_candidates:
                # ignora se dm já está contido dentro de um token maior
                if any(dm in big for big in tokens):
                    continue
                tokens.append(dm)
    
            # tenta também strings tipo "2023-05-18 00:00:00" (pega os 10 primeiros)
        ...
        # ============================================================
        # Cache: sheetnames + loader (agora extrai TODAS as datas por célula)
        # ============================================================
        @st.cache_data(show_spinner=False)
        ...
        def _load_bxl_events_from_excel(_file_path: str, sheet_name: str):
            ...
            for r in range(2, ws.max_row + 1):
                ...
                # eventos: PARA CADA COLUNA, extrair TODAS as datas da célula
                for bc in bxl_cols:
        ...
        # ============================================================
        # UI (somente futuras 0–30 + WhatsApp)
        # ============================================================
        st.markdown("## Agenda de Coletas (BxL) — Próximas (0–30 dias)")
        ...
        try:
            ...
            st.error(f"Não foi possível abrir a planilha: {file_path}\n\nDetalhe: {e}")
            return
        ...
        if ev.empty:
            st.info("Não há coletas futuras (0–30) para este CBL nos filtros atuais.")
            return
    
        ev["Label"] = ev.apply(lambda r: f"{r['Coleta_short']} — {r['Data_str']} (D-{int(r['Dias'])})", axis=1)
