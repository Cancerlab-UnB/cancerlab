...
# ---- PÁGINA INICIAL (após login) ----
    ...
    def painel_clinicos(df: pd.DataFrame):
        ...
        st.markdown("#### 🔎 Filtros (sem intervalo de datas)")
        with st.container():
    ...
    # =====================================================================
    # ============== PAINEL 2 — BIÓPSIA LÍQUIDA (agenda simplificada) =====
    # =====================================================================
    def painel_biopsia(df: pd.DataFrame):
        # Colunas de identificação (nomes EXATOS)
        col_nome   = _by_exact(df, ["Nome"])
        col_pront  = _by_exact(df, ["Prontuário"])
        col_seg    = _by_exact(df, ["Segmento"])
        col_idpac  = _by_exact(df, ["ID \npaciente"])
    
        # Eventos/colunas de data (EXATOS da planilha)
        ev_cols = [c for c in df.columns if any([
            c.startswith("BxL 1"), c.startswith("BxL 2"), c.startswith("BxL 3"),
            c.startswith("BxL 4"), c.startswith("BxL 5"), c.startswith("BxL 6"),
            c.startswith("BxL 7"), c.startswith("BxL 8"),
            c == "Bxl pós-RT", c == "CIRURGIA", c.startswith("bx liquida CIRURGIA")
        ])]
    
        # Coerção segura dessas colunas para datetime
        for c in ev_cols:
            df[c] = pd.to_datetime(df[c], errors="coerce", dayfirst=True)
    
        # Monta calendário longo (Evento, Data)
        id_cols = [c for c in [col_nome, col_pront, col_seg, col_idpac] if c]
        cal = df[id_cols + ev_cols].copy()
        cal = cal.melt(id_vars=id_cols, value_vars=ev_cols, var_name="Evento", value_name="Data")
        cal = cal[cal["Data"].notna()].sort_values("Data")
        ...
        with st.container():
            ...
            if not cal.empty:
                dmin, dmax = cal["Data"].min().date(), cal["Data"].max().date()
                dr = f1.date_input("Intervalo (todas as etapas BxL/Cirurgia/RT)", (dmin, dmax), min_value=dmin, max_value=dmax)
                if isinstance(dr, tuple) and len(dr)==2:
                    d0, d1 = pd.to_datetime(dr[0]), pd.to_datetime(dr[1]) + pd.Timedelta(days=1)
                    cal = cal[cal["Data"].between(d0, d1)]
    
            if col_seg:
                segs = f2.multiselect("Segmento", sorted(df[col_seg].dropna().astype(str).unique()))
                if segs:
                    cal = cal[cal[col_seg].astype(str).isin(segs)]
    
            # busca rápida por Nome/Prontuário
            q = f3.text_input("Buscar nome/prontuário")
            if q:
                qn = _norm_txt(q)
                mask = pd.Series([True]*len(cal))
                if col_nome:
                    mask &= cal[col_nome].fillna("").map(lambda x: qn in _norm_txt(x))
                if col_pront:
                    mask |= cal[col_pront].astype(str).fillna("").str.contains(q, case=False, na=False)
                cal = cal[mask]
    
            st.download_button("⬇️ Exportar CSV (agenda filtrada)",
                               data=cal.to_csv(index=False).encode("utf-8"),
                               file_name="agenda_bxl_filtrada.csv", mime="text/csv",
                               use_container_width=True)
            st.markdown('</div>', unsafe_allow_html=True)
    
        # ------- TABS -------
        tab_overview, tab_dist, tab_trend, tab_table = st.tabs(
            ["🗓️ Agenda & indicadores","📦 Distribuições","📈 Tendências","📋 Tabela"]
        )
    
        with tab_overview:
            n_evt = len(cal)
            prox7 = prox30 = prox60 = 0
            hoje = pd.Timestamp("today").normalize()
            prox7  = int(cal["Data"].between(hoje, hoje+pd.Timedelta(days=7), inclusive="right").sum())
            prox30 = int(cal["Data"].between(hoje, hoje+pd.Timedelta(days=30), inclusive="right").sum())
            prox60 = int(cal["Data"].between(hoje, hoje+pd.Timedelta(days=60), inclusive="right").sum())
    
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Eventos (filtrados)", f"{n_evt:,}".replace(",",".")); m2.metric("Próx. 7 dias", f"{prox7}")
            m3.metric("Próx. 30 dias", f"{prox30}"); m4.metric("Próx. 60 dias", f"{prox60}")
            ...
            if not cal.empty:
                ...
                c1.markdown("**Eventos por mês**"); c1.plotly_chart(fig, use_container_width=True)
    
                topE = _top_counts(cal["Evento"], top=20)
                fig2 = px.bar(topE, y="valor", x="contagem", orientation="h", text="contagem")
                fig2 = _apply_fig_base(fig2, h=300); fig2.update_traces(textposition="outside")
                c2.markdown("**Tipos de evento**"); c2.plotly_chart(fig2, use_container_width=True)
            else:
                st.info("Nenhum evento nas condições atuais.")
    
        with tab_dist:
            if cal.empty:
                st.info("Sem dados para distribuir.")
            else:
                alvo = st.selectbox("Coluna para distribuir", [c for c in cal.columns if c not in ["Data"]], index=0)
                tipo = st.radio("Visualização", ["Barras","Pizza (donut)"], horizontal=True)
                top = _top_counts(cal[alvo], top=30)
                if tipo == "Barras":
                    fig = px.bar(top, y="valor", x="contagem", orientation="h", text="contagem")
                    fig = _apply_fig_base(fig, h=420); fig.update_traces(textposition="outside")
                else:
        ...
        with tab_trend:
            if cal.empty:
                ...
                group = st.selectbox("Agrupar por (opcional)", ["(nenhum)"] + [c for c in cal.columns if c not in ["Data"]], index=0)
                dd = cal[["Data"]].copy()
                dd["t"] = dd["Data"].dt.to_period("M").dt.to_timestamp()
                if group != "(nenhum)":
                    dd[group] = cal[group].astype(str)
                    g = dd.groupby(["t",group]).size().reset_index(name="contagem")
                    fig = px.line(g, x="t", y="contagem", color=group, markers=True)
                else:
                    g = dd.groupby("t").size().reset_index(name="contagem")
        ...
        with tab_table:
            st.dataframe(cal.sort_values("Data"), use_container_width=True, hide_index=True, height=560)
    
    # =================== ROUTER ===================
    if "Biópsia" in fonte:
        st.markdown("### 🧪 Biópsia Líquida — Agenda & Acompanhamento (simplificado)")
        painel_biopsia(df)
    else:
        st.markdown("### 🧬 Dados Clínicos — Analytics")
        painel_clinicos(df)
    # =================== FIM ===================
