...
# PÁGINA INICIAL
    ...
    def painel_agenda_bxl(file_path: str = "Entrada e Acompanhamento de Pacientes Biópsia Líquida CCR.xlsx"):
        # =====================================================================
        # PAINEL AGENDA BxL — robusto (datas), registro em aba + marcação na célula
        #
        # Regras:
        # - Exibição atrasados: Dias entre -1 e -7 e Status != REGISTRADA => aparece na aba Atrasadas
        # - Automação: Dias == -1 e Status não REGISTRADA => grava ATRASADA automaticamente (persistente)
        # - Registro manual: funciona tanto para 0–30 quanto para -1 a -7
        # =====================================================================
        import re
        import unicodedata
        import os
        ...
        def _normalize_cbl(raw: str) -> str:
            s = str(raw or "").strip()
        ...
        def _is_in_study_id(raw: str) -> bool:
        ...
        RE_D_MON_Y = re.compile(
            r"(?<!\d)(\d{1,2})\s*(?:de\s*)?([A-Za-zÀ-ÿ\.]+)\s*(?:de\s*)?(\d{2,4})(?!\d)",
            flags=re.IGNORECASE
        )
    
        def _two_digit_year_to_yyyy(yy: int) -> int:
            return 2000 + yy if yy <= 69 else 1900 + yy
    
        def _infer_year_for_ddmm(d: int, m: int, today_: date) -> date | None:
            """
            dd/mm sem ano:
            - se for até 7 dias no passado => mantém (permite ontem, anteontem...)
            - se for mais antigo => assume próxima ocorrência (ano atual se ainda não passou; senão ano seguinte)
            """
            y = today_.year
            try:
                cand = date(y, m, d)
        ...
        def _extract_dates_from_string(s: str, today_: date) -> list[date]:
            if not s:
                return []
            s0 = str(s).strip()
            ...
            s1 = " " + s0.lower() + " "
            s1 = s1.replace("\\", "/")
            s1 = s1.replace("º", "").replace("ª", "")
            s1 = re.sub(r"[,\t]+", " ", s1)
    
            def _month_to_num(match):
                w = match.group(0).strip().lower()
                w2 = w.replace(",", "").strip()
                if w2 in MONTHS:
                    return f" {MONTHS[w2]:02d} "
                return match.group(0)
    
            s1 = re.sub(r"\b[A-Za-zÀ-ÿ\.]{3,}\b", _month_to_num, s1)
            s1 = re.sub(r"\s+", " ", s1).strip()
    
            tokens = []
    
            for y, mo, d in RE_YMD.findall(s1):
                tokens.append(("YMD", y, mo, d))
    
            for d, mo, y in RE_DMY.findall(s1):
                tokens.append(("DMY", d, mo, y))
    
            for d, mon_word, y in RE_D_MON_Y.findall(s0):
                mw = mon_word.strip().lower().replace(",", "")
                mnum = MONTHS.get(mw)
                if mnum:
                    tokens.append(("DMY", d, str(mnum), y))
    
            for m in RE_YMD_TIME.findall(s1):
                ds = m[0]
                m2 = re.search(r"(\d{4})[\/\-.](\d{1,2})[\/\-.](\d{1,2})", ds)
                if m2:
                    y, mo, d = m2.groups()
                    tokens.append(("YMD", y, mo, d))
    
            big_spans = []
            for m in re.finditer(r"\d{4}[\/\-.]\d{1,2}[\/\-.]\d{1,2}", s1):
                big_spans.append(m.span())
            for m in re.finditer(r"\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4}", s1):
                big_spans.append(m.span())
    
            def _inside_big(span):
                for a, b in big_spans:
                    if span[0] >= a and span[1] <= b:
                        return True
                return False
    
            for m in RE_DM.finditer(s1):
                if not _inside_big(m.span()):
                    d, mo = m.groups()
                    tokens.append(("DM", d, mo, ""))
    
            out = []
            seen = set()
    
            for kind, a, b, c in tokens:
                dd = None
                try:
                    if kind == "YMD":
                        y = int(a); mo = int(b); d = int(c)
                        dd = date(y, mo, d)
                    elif kind == "DMY":
                        d = int(a); mo = int(b)
                        yraw = str(c).strip()
                        y = _two_digit_year_to_yyyy(int(yraw)) if len(yraw) == 2 else int(yraw)
                        dd = date(y, mo, d)
                    elif kind == "DM":
                        d = int(a); mo = int(b)
                        dd = _infer_year_for_ddmm(d, mo, today_)
                except Exception:
                    dd = None
    
                if dd and _valid_year(dd) and dd not in seen:
                    seen.add(dd)
                    out.append(dd)
    
            return out
    
        def _extract_all_dates(x, today_: date, epoch) -> list[date]:
            if x is None or (isinstance(x, float) and np.isnan(x)):
                return []
    
            if isinstance(x, datetime):
                d = x.date()
                return [d] if _valid_year(d) else []
    
            if isinstance(x, date):
                return [x] if _valid_year(x) else []
            ...
            if not s:
                return []
            return _extract_dates_from_string(s, today_)
    
        def _is_simple_date_string(raw: str) -> bool:
            if raw is None:
                return False
            s = str(raw).strip()
            if not s:
            ...
            if re.fullmatch(r"\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4}", s2):
                return True
            if re.fullmatch(r"\d{1,2}[\/\-.]\d{1,2}", s2):
                return True
            return False
    
        # ============================================================
        # Registro + marcação em célula
        # ============================================================
        ...
        if "bxl_work_path" not in st.session_state:
            st.session_state["bxl_work_path"] = file_path
        ...
        st.session_state["bxl_work_path"] = _ensure_writable_workbook_path(st.session_state["bxl_work_path"])
        if st.session_state["bxl_work_path"] != file_path:
            st.warning(
                "⚠️ O arquivo original não está gravável (bloqueado/sem permissão). "
                "Vou salvar e sobrescrever em uma cópia editável:\n\n"
                f"**{st.session_state['bxl_work_path']}**"
            )
            file_path = st.session_state["bxl_work_path"]
        ...
        @st.cache_data(show_spinner=False)
        def _load_registro_cached(_file_path: str, _mtime: float, _v: int):
            try:
        ...
        def _load_registro_nocache(_file_path: str):
            try:
                wb = openpyxl.load_workbook(_file_path, data_only=True)
                if REG_SHEET not in wb.sheetnames:
                    return pd.DataFrame(columns=REG_HEADERS)
        ...
        def _status_map(df_reg: pd.DataFrame) -> dict:
            m = {}
        ...
        def _apply_cell_mark(ws, cell_ref: str, d: date, status: str, motivo: str, obs: str, keep_raw: bool, raw_value):
            cell = ws[cell_ref]
            stt = (status or "").upper()
    
            # Só sobrescreve value se for "seguro"
            if not keep_raw:
        ...
        def _upsert_registro_and_update_cell(_file_path: str, rec: dict):
            wb = openpyxl.load_workbook(_file_path)
            ...
            cell_ref = rec.get("Cell")
            dd = rec.get("_date_obj")
            keep_raw = bool(rec.get("_keep_raw", False))
            raw_value = rec.get("_raw_value")
    
            if sheet_name in wb.sheetnames and cell_ref and isinstance(dd, date) and _valid_year(dd):
                try:
                    ws = wb[sheet_name]
                    _apply_cell_mark(ws, cell_ref, dd, rec.get("Status", ""), rec.get("Motivo", ""), rec.get("Obs", ""),
                                     keep_raw, raw_value)
                except Exception:
                    pass
    
            try:
                _atomic_save_workbook(wb, _file_path)
                return _file_path
            except PermissionError:
                base, ext = os.path.splitext(_file_path)
                alt = f"{base}__EDITAVEL{ext}"
                try:
                    shutil.copy2(_file_path, alt)
                except Exception:
                    alt = f"REGISTRO__EDITAVEL__{uuid.uuid4().hex}.xlsx"
                _atomic_save_workbook(wb, alt)
                st.session_state["bxl_work_path"] = alt
                st.warning(
                    "⚠️ O Excel original está bloqueado/sem permissão. "
                    "Salvei e vou continuar sobrescrevendo na cópia editável:\n\n"
                    f"**{alt}**"
                )
                return alt
    
        # ============================================================
        # Leitura do Excel -> eventos
        # ============================================================
        @st.cache_data(show_spinner=False)
        def _get_sheetnames(_file_path: str, _mtime: float):
            wb = openpyxl.load_workbook(_file_path, data_only=True)
            return wb.sheetnames
    
        @st.cache_data(show_spinner=False)
        def _load_events(_file_path: str, _mtime: float, sheet_name: str):
            wb = openpyxl.load_workbook(_file_path, data_only=True)
            ...
            for r in range(2, ws.max_row + 1):
                ...
                for bc in bxl_cols:
                    ...
                    dates = _extract_all_dates(raw, today_=today_, epoch=epoch)
                    if not dates:
                        continue
    
                    raw_str = "" if raw is None else str(raw).strip()
                    keep_raw = isinstance(raw, str) and not _is_simple_date_string(raw_str)
    
                    for dd in dates:
                        if not _valid_year(dd):
                            continue
                        events.append(
                            {
                                "CBL": cbl,
            ...
            df = df.drop_duplicates(subset=["CBL", "Coleta_short", "Data", "Sheet", "Cell"], keep="first").reset_index(drop=True)
            return df, phones_map, headers, phone_cols
        ...
        st.caption("Inclui: Atrasadas (-1 a -7), Próximas (0–30), Registro e WhatsApp.")
    
        try:
            sheetnames = _get_sheetnames(file_path, _file_mtime(file_path))
            if not sheetnames:
                st.error("A planilha não contém abas.")
                return
        except Exception as e:
            st.error(f"Não foi possível abrir a planilha:\n\n{file_path}\n\nDetalhe: {e}")
            return
    
        st.sidebar.markdown("### Filtros")
        sheet = st.sidebar.selectbox("Aba da planilha", sheetnames, index=0)
        cbl_search = st.sidebar.text_input("Buscar CBL (contém)", value="").strip()
        hide_withdrawn = st.sidebar.checkbox("Ocultar retirados do estudo (ID != CBL123)", value=True)
    
        df_all, phones_map, headers, phone_cols = _load_events(file_path, _file_mtime(file_path), sheet)
        if df_all.empty:
            st.warning("Nenhuma data BxL encontrada nesta aba (ou todas estão vazias).")
            return
    
        today_ = date.today()
        df_all["Dias"] = df_all["Data"].apply(lambda d: (d - today_).days if isinstance(d, date) else np.nan)
    
        # Status via registro
        df_reg = _load_registro_cached(file_path, _file_mtime(file_path), st.session_state["reg_version_bxl"])
        smap = _status_map(df_reg)
    
        def _key_status(r) -> str:
            return f"{str(r['CBL']).strip()}||{str(r['Coleta_short']).strip()}||{r['Data'].isoformat()}"
    
        df_all["KeyStatus"] = df_all.apply(_key_status, axis=1)
        df_all["Status"] = df_all["KeyStatus"].apply(lambda k: smap.get(k, "PENDENTE")).astype(str).str.upper()
    
        # ============================================================
        # Automação D-1: marca ATRASADA se não REGISTRADA (persistente)
        # ============================================================
        ...
        if not auto.empty:
            ...
            for _, r in auto.iterrows():
                rec = {
                    ...
                    "Obs": "",
                    "Updated_at": now,
                    "Sheet": str(r.get("Sheet", sheet)),
                    "Cell": str(r.get("Cell", "")),
                    "_date_obj": r["Data"],
                    "_keep_raw": bool(r.get("_keep_raw", False)),
                    "_raw_value": r.get("_raw_value", None),
                }
                try:
                    new_path = _upsert_registro_and_update_cell(file_path, rec)
                    if new_path != file_path:
                        file_path = new_path
                except Exception as e:
                    st.error(f"Falha ao salvar auto-atraso: {e}")
    
            st.session_state["bxl_auto_guard"]["keys"].update(set(attempted))
    
            df_check = _load_registro_nocache(file_path)
            smap_check = _status_map(df_check)
            verified = sum(1 for k in attempted if smap_check.get(k) == "ATRASADA")
    
            if verified > 0:
                st.session_state["reg_version_bxl"] += 1
                st.toast(f"{verified} coleta(s) marcadas como ATRASADAS automaticamente (D-1).", icon="⏰")
                st.rerun()
            else:
                st.warning(
                    "⚠️ Tentei marcar D-1 como ATRASADA, mas não consegui confirmar a gravação no Excel. "
                    "Feche o Excel (se estiver aberto) e tente novamente."
                )
        ...
        if df.empty:
            st.info("Após os filtros, não restaram pacientes para exibir.")
            return
    
        # ============================================================
        # CRITÉRIO DE ATRASO PARA EXIBIÇÃO (independe do registro):
        # -1 a -7 e não REGISTRADA => aparece na aba de atrasados
        # ============================================================
        ...
        def _prefix_status(stt: str, coleta: str) -> str:
            stt = (stt or "").upper()
            if stt == "ATRASADA":
                return f"⏰ {coleta}"
            if stt == "REGISTRADA":
        ...
        # ============================================================
        # Recortes:
        # - atrasadas: -7..-1 (não registrada)
        # - futuras: 0..30
        # ============================================================
        atras = df[df["AtrasadaExibicao"] == True].copy()
        atras = atras.sort_values(["Dias", "Data", "CBL", "Coleta_short"]).reset_index(drop=True)
    
        fut = df[(df["Dias"] >= 0) & (df["Dias"] <= 30)].copy()
        ...
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Atrasadas (-1 a -7)", int(len(atras)))
        ...
        # Abas
        # ============================================================
        tabA, tab1, tab2, tab3 = st.tabs(["Atrasadas (-1 a -7)", "0–7 dias", "8–14 dias", "15–30 dias"])
    
        SHOW_COLS_FUT = ["CBL", "Coleta_show", "Data_str", "Dias", "Status_display"]
        SHOW_COLS_ATR = ["CBL", "Coleta_show", "Data_str", "Dias", "Status_display"]
    
        with tabA:
            st.caption("Inclui: ATRASADA (registrada) e PENDENTE EM ATRASO (ainda sem registro).")
            if atras.empty:
                st.info("Nenhuma coleta em atraso entre -1 e -7 dias.")
            else:
                st.dataframe(
                    atras[SHOW_COLS_ATR].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status"}),
        ...
        with tab1:
            if fut_0_7.empty:
                ...
                st.dataframe(
                    fut_0_7[SHOW_COLS_FUT].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status"}),
                    use_container_width=True, hide_index=True, height=420
                )
        ...
        with tab2:
            if fut_8_14.empty:
                ...
                st.dataframe(
                    fut_8_14[SHOW_COLS_FUT].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status"}),
                    use_container_width=True, hide_index=True, height=420
                )
        ...
        with tab3:
            if fut_15_30.empty:
                ...
                st.dataframe(
                    fut_15_30[SHOW_COLS_FUT].rename(columns={"Coleta_show": "Coleta", "Status_display": "Status"}),
                    use_container_width=True, hide_index=True, height=420
                )
    
        # ============================================================
        # Registro manual: agora usa (atrasadas + futuras)
        # ============================================================
        st.divider()
        st.markdown("### Registro de coleta")
        st.caption("Funciona para atrasadas (-1 a -7) e futuras (0–30).")
        ...
        action_df["LabelUI"] = action_df.apply(
            lambda r: f"{r['CBL']} — {r['Coleta_short']} — {r['Data_str']} — {r['Status_display']} (D{int(r['Dias'])})",
            axis=1
        )
        ...
        obs_reg = st.text_input("Observação (opcional)", value="").strip()
        b1, b2, b3 = st.columns([1.0, 1.0, 1.6])
    
        def _set_status(new_status: str, motivo: str):
            nonlocal file_path
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            rec = {
                "CBL": str(sel_row["CBL"]).strip(),
                "Coleta_short": str(sel_row["Coleta_short"]).strip(),
                "Data_iso": sel_row["Data"].isoformat(),
                "Status": str(new_status).upper(),
                "Motivo": motivo,
                "Obs": obs_reg,
                "Updated_at": now,
                "Sheet": str(sel_row.get("Sheet", sheet)),
                "Cell": str(sel_row.get("Cell", "")),
                "_date_obj": sel_row["Data"],
                "_keep_raw": bool(sel_row.get("_keep_raw", False)),
                "_raw_value": sel_row.get("_raw_value", None),
            }
            new_path = _upsert_registro_and_update_cell(file_path, rec)
            if new_path != file_path:
                file_path = new_path
        ...
        with b1:
            if st.button("✅ Registrar coleta", use_container_width=True):
                try:
                    _set_status("REGISTRADA", "Manual: registrada no painel")
                    st.toast("Coleta registrada e célula marcada.", icon="✅")
                    st.rerun()
                except Exception as e:
                    st.error(f"Erro ao salvar registro: {e}")
    
        with b2:
            if st.button("⏰ Marcar como atrasado", use_container_width=True):
                try:
                    _set_status("ATRASADA", "Manual: marcada como atrasada no painel")
                    st.toast("Atraso marcado e célula marcada.", icon="⏰")
                    st.rerun()
                except Exception as e:
                    st.error(f"Erro ao salvar atraso: {e}")
    
        with b3:
            st.caption(
                f"CBL: {sel_row['CBL']} | Coleta: {sel_row['Coleta_short']} | Data: {sel_row['Data_str']} | "
                f"Dias: {int(sel_row['Dias'])} | Status: {sel_row['Status_display']}"
            )
    
        # ============================================================
        # WhatsApp (usa futuras 0–30 por padrão)
        # ============================================================
        st.divider()
        st.markdown("### WhatsApp — Enviar lembrete")
        st.caption("Por padrão, usa apenas coletas futuras (0–30).")
    
        if fut.empty:
            st.info("Sem coletas futuras (0–30) para WhatsApp com os filtros atuais.")
            return
        ...
        with w1:
            cbl_options = sorted(fut["CBL"].dropna().astype(str).unique().tolist())
        ...
        ev = fut[fut["CBL"].astype(str) == str(sel_cbl)].copy().sort_values(["Data"]).reset_index(drop=True)
        if ev.empty:
            st.info("Não há coletas futuras (0–30) para este CBL.")
            return
    
        ev["Label"] = ev.apply(lambda r: f"{r['Coleta_short']} — {r['Data_str']} — {r['Status_display']} (D{int(r['Dias'])})", axis=1)
        ...
        if phones_e164:
            ...
            sel_idx = st.selectbox("Contato (telefone)", options=list(range(len(display_opts))), format_func=lambda i: display_opts[i], index=0)
            phone_e164 = phones_e164[int(sel_idx)][0]
            phone_disp = _format_phone_br(phones_e164[int(sel_idx)][1])
        else:
            st.warning("Não encontrei telefone válido no campo selecionado. Cole um número manualmente.")
            manual = st.text_input("Telefone manual (DDD + número ou com 55)", value="")
            phone_e164 = _normalize_to_e164_br(manual)
            phone_disp = _format_phone_br(manual) if manual else ""
    
        st.markdown("#### Mensagem")
        template = st.selectbox("Modelo", ["Lembrete padrão", "Confirmação de presença", "Solicitar reagendamento", "Mensagem neutra"], index=0)
        ...
        if template == "Lembrete padrão":
            ...
            default_msg = (
                f"Olá! Sobre a coleta {coleta_txt} prevista para {data_txt}: "
                "você precisa reagendar? Se sim, me informe sua disponibilidade."
            )
        else:
            default_msg = "Olá! Estou entrando em contato para confirmar um compromisso de coleta. Pode me responder por aqui, por favor?"
        ...
        with st.expander("Campos brutos", expanded=False):
            st.write(f"Campo de contato bruto na planilha ({sel_phone_col}):")
            st.code(str(raw_contact))
            st.write("Valor bruto do campo de data (célula original):")
