...
MAX_ROWS_GLOBAL = 120  # ajuste aqui (100–200 ideal)

# salva original — only patch once to avoid re-wrapping on every Streamlit rerun
if not getattr(st, "_dataframe_patched", False):
    _original_dataframe = st.dataframe

    def safe_dataframe(df, *args, **kwargs):
        try:
            if isinstance(df, pd.DataFrame) and len(df) > MAX_ROWS_GLOBAL:
                st.caption(f"⚠ Mostrando apenas {MAX_ROWS_GLOBAL} de {len(df)} linhas")
                df = df.head(MAX_ROWS_GLOBAL)
        except Exception:
            pass
...
def _set_param(**kwargs):
    # Only update query params when a value actually changed, and never during
    # the initial render pass (which would cause an immediate extra rerun).
    try:
        current = dict(st.query_params)
        changed = any(current.get(k) != v for k, v in kwargs.items())
        if changed:
...
def page_publications():
    ...
    PUBLICATIONS = [
      ...
      {
       "authors": "TELLES, L.A. ; DE LOYOLA, M.B. ; SAKAMOTO, L.T. ; RABELLO, D.D. ; MOTOYAMA, A.B. ; PITTELLA-SILVA, F.",
       "title": "SETD4 as a marker of disease burden and treatment response in childhood acute lymphoblastic leukemia",
       "journal": "Oncology Letters",
       "year": 2026,
       "url": "https://doi.org/10.3892/ol.2026.15545",
       "doi": "10.3892/ol.2026.15545"
       },
      {
      "authors": "BICALHO, L.O. ; CAVALCANTE, C.Q.O. ; SANTANA, B.L.O. ; GUALBERTO, A.C.M. ; PITTELLA-SILVA, F. ; NASCIMENTO, É.C.M. ; MARTINS, J.B.L. ; GATTO, C.C.",
...
def page_equipment():
    """
    Equipments page (no search bar)
    - Responsive card grid
    - Full-card click -> PNIPE equipment page in a new tab
    - Schedule Equipment button for external reservations
    """

    # Handle scheduling sub-page
    if st.session_state.get("page") == "equipment_pub":
        # Check if user clicked "Schedule Equipment"
        pass  # we route below
    ...
    <style>
      ...
      .eq-sched-banner {
        ...
        background:linear-gradient(135deg,#1a3fa8 0%,#2a5fc8 100%);
        border-radius:14px; padding:18px 22px; margin-bottom:20px;
        box-shadow:0 8px 20px rgba(26,63,168,.28);
      }
      .eq-sched-text { color:#fff; }
      .eq-sched-text strong { font-size:17px; display:block; font-weight:900; margin-bottom:3px; }
      .eq-sched-text span { font-size:13.5px; opacity:.88; }
    ...
    st.markdown('<div class="eq-h1">Equipments</div>', unsafe_allow_html=True)

    # ---- Schedule Equipment banner ----
    st.markdown("""
    <div class="eq-sched-banner">
        <div class="eq-sched-text">
            <strong>📅 Need to use lab equipment?</strong>
            <span>Schedule your session online — quick registration, instant confirmation.</span>
        </div>
    </div>
    """, unsafe_allow_html=True)

    if st.button("📅 Schedule Equipment", key="eq_schedule_btn",
                 help="Open the equipment scheduling portal for external users"):
        st.session_state.page = "schedule_equipment"
        st.rerun()

    st.markdown('<div class="eq-sub" style="margin-top:12px;">Click any card to open the PNIPE equipment page.</div>', unsafe_allow_html=True)
...
def page_people():
    ...
    PEOPLE = [
        ...
        "lattes": "http://lattes.cnpq.br/2380982097052834", "orcid": "https://orcid.org/0000-0001-5896-7204"},
           ...
           {"name": "Mariana Braccialli de Loyola, MSc", "role": "PhD Student",
            "photo": "Mari.jpg", "bio": "Pharmacist graduated from the University of Brasília (UnB). Holds a master’s degree from the Graduate Program in Medical Sciences at the same institution. She is currently a PhD candidate in the Graduate Program in Medical Sciences at the University of Brasília, under the supervision of Prof. Dr. Fabio Pittella. She works in the Laboratory of Molecular Pathology of Cancer, conducting research in the field of cancer molecular biology. During her master’s studies, she investigated the role of methyltransferases in the context of acute lymphoblastic leukemia development. In her PhD, she is developing the project Functional Evaluation of PSMA in Advanced Prostate Cancer Using CRISPR/Cas9 Models: Metabolic and Angiogenic Implications, with a focus on the functional characterization of PSMA and its effects on tumor metabolism and angiogenesis.",
        ...
        {"name": "Alvaro Reis de Oliveira Queiroz", "role": "Undergraduate Student",
         "photo": "mtl.jpeg", "bio": "Bachelor’s student in Pharmacy at the University of Brasília (UnB). Experienced with coordination chemistry and applications to biological systems. Working with the generation of stable Cas9-EGFP cancer cell lines, using the Tet-On system.",
        ...
        {"name": "Gabriel da Silva Macêdo", "role": "Undergraduate Student",
         "photo": "gabrielft.jpg", "bio": "Undergraduate student in Pharmacy at the University of Brasília (UnB), currently in the 6th semester. Interested in research in the fields of molecular biology and genetics, with an emphasis on applications related to human health. Intends to continue academic training through undergraduate research programs and graduate studies.",
         "degrees": ["B.Sc. student in Pharmacy, University of Brasília (2019–present)"], "email": "", "institution": "University of Brasília",
    ...
    ALUMINI = [
         ...
         {"name": "Mariana Braccialli de Loyola", "lattes": "http://lattes.cnpq.br/4480047944308175", "year": "2025", "activity": "M.Sc."},
         ...
         {"name": "Lara De Souza Moreno", "lattes": "http://lattes.cnpq.br/4568437840946460", "year": "2026", "activity": "M.Sc."},
...
usuarios_table = Table(
    ...
    Column('perfil', String, nullable=False),
)
...
def garantir_tabelas():
    """
    Garante que as tabelas existam antes de consultar ou inserir.
    Atenção: create_all cria tabelas novas, mas não altera colunas de tabelas antigas.
    """
...
ADMIN_EMAIL = "victor.fleck906438@gmail.com"
LAB_EMAIL = "cancerlab@unb.br"      # primary notification target per requirement
NOREPLY_EMAIL = SMTP_USER            # uses the same SMTP user as noreply sender

external_users_table = Table(
    ...
    Column('fub_registration', String, nullable=False),
    Column('created_at', DateTime, default=datetime.utcnow),
)

# Status values:
#   'pending'   – newly submitted, awaiting internal review  (NEW)
#   'approved'  – approved by an internal user               (NEW)
#   'rejected'  – rejected by an internal user               (NEW)
#   'cancelled' – cancelled by the requester or by an admin  (EXISTING)
# Legacy rows with 'confirmed' are treated as 'approved' everywhere in display logic.
bookings_table = Table(
    'equipment_bookings', metadata,
    Column('id', Integer, primary_key=True),
    Column('external_user_id', Integer, nullable=False),
    Column('user_name', String, nullable=False),
    Column('user_email', String, nullable=False),
    Column('user_phone', String, nullable=False),
    Column('user_fub', String, nullable=False),
    Column('equipment_name', String, nullable=False),
    Column('booking_date', String, nullable=False),   # stored as ISO string YYYY-MM-DD
    Column('booking_time', String, nullable=False),   # stored as HH:MM
    Column('status', String, nullable=False, default='pending'),
    Column('notes', String, nullable=True),
    Column('reviewed_by', String, nullable=True),     # internal user name who reviewed
    Column('reviewed_at', DateTime, nullable=True),   # when reviewed
    Column('review_note', String, nullable=True),     # optional note from reviewer
    Column('cancelled_by', String, nullable=True),    # 'user' or internal user name
    Column('cancelled_at', DateTime, nullable=True),
    Column('created_at', DateTime, default=datetime.utcnow),
)

# Audit log for all internal actions on bookings
booking_audit_table = Table(
    'booking_audit_log', metadata,
    Column('id', Integer, primary_key=True),
    Column('booking_id', Integer, nullable=False),
    Column('action', String, nullable=False),         # 'submitted','approved','rejected','cancelled'
    Column('performed_by', String, nullable=False),   # 'user:<name>' or 'internal:<name>'
    Column('note', String, nullable=True),
    Column('created_at', DateTime, default=datetime.utcnow),
)


def garantir_tabelas_schedule():
    metadata.create_all(engine, checkfirst=True)
    # Migrate legacy table: add new columns if missing (non-destructive ALTER)
    _migrate_bookings_table()


def _migrate_bookings_table():
    """Add new columns to equipment_bookings if they don't exist (safe for production)."""
    new_columns = [
        ("reviewed_by",  "VARCHAR"),
    ...
    try:
                ...
                try:
                    conn.execute(
                        __import__('sqlalchemy').text(
                            f"ALTER TABLE equipment_bookings ADD COLUMN IF NOT EXISTS {col_name} {col_type}"
                        )
                    )
                    conn.commit()
                except Exception:
        ...
        pass  # silently skip if DB doesn't support ALTER or table doesn't exist yet


garantir_tabelas_schedule()


# ---- External user helpers ----

def ext_user_email_exists(email: str) -> bool:
    garantir_tabelas_schedule()
    with engine.connect() as conn:
        res = conn.execute(
            select(external_users_table).where(external_users_table.c.email == email.lower().strip())
        )
        return res.fetchone() is not None


def salvar_ext_user(full_name, email, password, phone, fub):
    garantir_tabelas_schedule()
    hashed = bcrypt.hashpw(password.encode(), bcrypt.gensalt()).decode()
    with engine.begin() as conn:
        conn.execute(external_users_table.insert().values(
            full_name=full_name.strip(),
            email=email.lower().strip(),
            password_hash=hashed,
            phone=phone.strip(),
            fub_registration=fub.strip(),
            created_at=datetime.utcnow(),
        ))


def autentica_ext_user(email: str, password: str):
    garantir_tabelas_schedule()
    with engine.connect() as conn:
        res = conn.execute(
            select(external_users_table).where(external_users_table.c.email == email.lower().strip())
        ...
        if row:
            d = dict(row._mapping)
            if bcrypt.checkpw(password.encode(), d["password_hash"].encode()):
                return d
    return None
...
def is_slot_taken(equipment: str, booking_date: str, booking_time: str, exclude_id: int = None) -> bool:
    garantir_tabelas_schedule()
    with engine.connect() as conn:
        # A slot is occupied if there's an active booking (pending, approved, or legacy confirmed).
        # Cancelled and rejected bookings free the slot.
        q = select(bookings_table).where(
            (bookings_table.c.equipment_name == equipment) &
            (bookings_table.c.booking_date == booking_date) &
            (bookings_table.c.booking_time == booking_time) &
            (bookings_table.c.status.in_(['pending', 'approved', 'confirmed']))
        )
        if exclude_id:
            q = q.where(bookings_table.c.id != exclude_id)
        res = conn.execute(q)
        return res.fetchone() is not None


def salvar_booking(ext_user_id, user_name, user_email, user_phone, user_fub, equipment, date_str, time_str, notes=""):
    garantir_tabelas_schedule()
    with engine.begin() as conn:
        result = conn.execute(bookings_table.insert().values(
            ...
            status='pending',           # All new bookings start as pending approval
            notes=notes,
            created_at=datetime.utcnow(),
        ))
        bid = result.inserted_primary_key[0]
        # Audit log
        conn.execute(booking_audit_table.insert().values(
            booking_id=bid,
            action='submitted',
            performed_by=f'user:{user_name}',
...
def get_user_bookings(ext_user_id: int):
    garantir_tabelas_schedule()
    with engine.connect() as conn:
        res = conn.execute(
            select(bookings_table).where(bookings_table.c.external_user_id == ext_user_id).order_by(bookings_table.c.booking_date.desc(), bookings_table.c.booking_time.desc())
        )
...
def get_all_bookings():
    garantir_tabelas_schedule()
    with engine.connect() as conn:
        res = conn.execute(select(bookings_table).order_by(bookings_table.c.booking_date.desc(), bookings_table.c.booking_time.desc()))
        return [dict(r._mapping) for r in res]


def cancel_booking(booking_id: int, cancelled_by: str = "admin"):
    garantir_tabelas_schedule()
    with engine.begin() as conn:
        conn.execute(
...
def approve_booking(booking_id: int, reviewed_by: str, review_note: str = "") -> dict:
    """Approve a pending booking. Returns the booking dict after update."""
    garantir_tabelas_schedule()
    with engine.begin() as conn:
        conn.execute(
            bookings_table.update().where(bookings_table.c.id == booking_id).values(
        ...
        conn.execute(booking_audit_table.insert().values(
            booking_id=booking_id,
            action='approved',
            performed_by=f'internal:{reviewed_by}',
...
def reject_booking(booking_id: int, reviewed_by: str, review_note: str = "") -> dict:
    """Reject a pending booking. Returns the booking dict after update."""
    garantir_tabelas_schedule()
    with engine.begin() as conn:
        conn.execute(
            bookings_table.update().where(bookings_table.c.id == booking_id).values(
...
def get_booking_by_id(booking_id: int) -> dict | None:
    garantir_tabelas_schedule()
    with engine.connect() as conn:
        res = conn.execute(select(bookings_table).where(bookings_table.c.id == booking_id))
        row = res.fetchone()
        return dict(row._mapping) if row else None


def get_audit_log_for_booking(booking_id: int) -> list:
    garantir_tabelas_schedule()
    with engine.connect() as conn:
        res = conn.execute(
            select(booking_audit_table)
            .where(booking_audit_table.c.booking_id == booking_id)
            .order_by(booking_audit_table.c.created_at.asc())
        )
        return [dict(r._mapping) for r in res]
...
def get_all_audit_logs() -> list:
    ...
    with engine.connect() as conn:
        res = conn.execute(
            select(booking_audit_table).order_by(booking_audit_table.c.created_at.desc())
        )
        return [dict(r._mapping) for r in res]
...
def get_pending_bookings() -> list:
    ...
    with engine.connect() as conn:
        res = conn.execute(
            ...
            .where(bookings_table.c.status == 'pending')
            .order_by(bookings_table.c.created_at.asc())
        )
        return [dict(r._mapping) for r in res]


def get_upcoming_approved_bookings() -> list:
    """Returns approved/confirmed/pending bookings from today onward for the shared calendar."""
    garantir_tabelas_schedule()
    from datetime import timezone, timedelta as _td
    today_str = str(datetime.now(timezone(_td(hours=-3))).date())
    with engine.connect() as conn:
        res = conn.execute(
            select(bookings_table)
            .where(bookings_table.c.status.in_(['approved', 'confirmed', 'pending']))
            .where(bookings_table.c.booking_date >= today_str)
            .order_by(bookings_table.c.booking_date.asc(), bookings_table.c.booking_time.asc())
        )
        return [dict(r._mapping) for r in res]
...
_email_logger = _logging.getLogger("cancerlab.email")
_email_logger.setLevel(_logging.DEBUG)
if not _email_logger.handlers:
    _email_handler = _logging.StreamHandler()
    _email_handler.setFormatter(_logging.Formatter("[%(asctime)s] %(levelname)s %(name)s: %(message)s"))
    _email_logger.addHandler(_email_handler)


def _send_email_safe(to: str, subject: str, body: str) -> tuple[bool, str]:
    """Send email with full error logging. Returns (success, error_message)."""
    # Validate SMTP credentials before attempting
    if not SMTP_USER or not SMTP_PASS:
        msg = (
            f"Email NOT sent to {to!r}: SMTP credentials missing. "
            f"Set SMTP_USER and SMTP_PASS environment variables "
            f"(SMTP_HOST={SMTP_HOST}, SMTP_PORT={SMTP_PORT})."
        )
        _email_logger.warning(msg)
        return False, "SMTP credentials not configured (SMTP_USER / SMTP_PASS missing)."

    if not to or "@" not in to:
        msg = f"Email NOT sent: invalid recipient address {to!r}."
        _email_logger.error(msg)
        return False, msg

    try:
        _email_logger.info(
            "Attempting to send email | to=%r | subject=%r | smtp=%s:%s | user=%s",
            to, subject, SMTP_HOST, SMTP_PORT, SMTP_USER,
        )
        msg_obj = EmailMessage()
        msg_obj["From"] = f"CancerLab – No Reply <{NOREPLY_EMAIL}>"
        msg_obj["To"] = to
        ...
        _email_logger.info("Email sent successfully to %r | subject=%r", to, subject)
        return True, ""

    except smtplib.SMTPAuthenticationError as e:
        err = f"SMTP authentication failed for user {SMTP_USER!r}: {e}"
        _email_logger.error(err)
        return False, err
    except smtplib.SMTPRecipientsRefused as e:
        err = f"Recipient refused by SMTP server: {to!r} — {e}"
        _email_logger.error(err)
        return False, err
    except smtplib.SMTPException as e:
        err = f"SMTP error sending to {to!r}: {e}"
        _email_logger.error(err)
        return False, err
    except OSError as e:
        err = f"Network/OS error connecting to {SMTP_HOST}:{SMTP_PORT} — {e}"
        _email_logger.error(err)
        return False, err
    except Exception as e:
        err = f"Unexpected error sending email to {to!r}: {type(e).__name__}: {e}"
        _email_logger.exception(err)
        return False, err


def send_booking_emails(booking: dict) -> dict:
    """Send 'pending approval' notification to user + internal staff. Returns dict with delivery status."""
    date_fmt = booking.get("booking_date", "")
    time_fmt = booking.get("booking_time", "")
    equip = booking.get("equipment_name", "")
    name = booking.get("user_name", "")
    email = booking.get("user_email", "")
    phone = booking.get("user_phone", "")
    fub = booking.get("user_fub", "")
    bid = booking.get("id", "")
    notes = booking.get("notes", "") or "—"

    # Determine user type: if external_user_id exists it's External, otherwise Internal
    user_type = "Usuário Externo"

    body_user = (
        f"Dear {name},\n\n"
        f"Your equipment booking request has been received by CancerLab – University of Brasília.\n\n"
        f"⏳ STATUS: PENDING APPROVAL\n"
        f"An authorized member of the laboratory team will review your request and you\n"
        f"will receive a notification email once a decision has been made.\n\n"
        f"━━━━━━ Request Details ━━━━━━━━━━━━━━━━━━━━━━\n"
        f"  Booking ID       : #{bid}\n"
        f"  Equipment        : {equip}\n"
        f"  Date             : {date_fmt}\n"
        ...
        f"Your Details:\n"
        f"  Name             : {name}\n"
        f"  FUB Registration : {fub}\n"
        f"  Phone            : {phone}\n"
        f"  User Type        : {user_type}\n\n"
        f"If you have any questions, please contact the laboratory at cancerlab@unb.br.\n\n"
        f"Best regards,\n"
        f"CancerLab – Laboratory of Molecular Pathology of Cancer\n"
        f"University of Brasília\n"
        f"This is an automated message. Please do not reply to this email."
    )

    body_lab = (
        f"⚠️ NEW BOOKING REQUEST — Awaiting Your Review\n\n"
        f"A new equipment booking request has been submitted and requires approval.\n\n"
        f"━━━━━━ Booking Details ━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"  Booking ID       : #{bid}\n"
        f"  Equipment        : {equip}\n"
        f"  Date             : {date_fmt}\n"
        ...
        f"  User Type        : {user_type}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Log in to the CancerLab internal panel to approve or reject this request."
    )

    # Send to requester
    ok_user, err_user = _send_email_safe(
        email,
        f"[CancerLab] Booking Request Received – #{bid} ({equip} on {date_fmt})",
        body_user,
    )
    # Send to lab (cancerlab@unb.br)
    ok_lab, err_lab = _send_email_safe(
        LAB_EMAIL,
        f"[CancerLab] ⚠️ New Booking #{bid} Awaiting Approval – {equip} on {date_fmt}",
        body_lab,
    )
    # Also notify admin backup
    ok_admin, err_admin = _send_email_safe(
        ADMIN_EMAIL,
        f"[CancerLab] New Booking #{bid} Pending – {equip} on {date_fmt}",
        body_lab,
    )

    _email_logger.info(
        "send_booking_emails result | booking_id=%s | user_ok=%s | lab_ok=%s | admin_ok=%s",
        bid, ok_user, ok_lab, ok_admin,
    )
    ...
    return {
        ...
        "lab_error": err_lab,
        "admin_sent": ok_admin,
        "admin_error": err_admin,
    }


def send_approval_decision_emails(booking: dict, decision: str, review_note: str = "") -> dict:
    """Send approval/rejection notification to requester AND lab staff.

    Args:
        booking: full booking dict
        decision: 'approved' or 'rejected'
        review_note: optional note from reviewer
    """
    date_fmt = booking.get("booking_date", "")
    time_fmt = booking.get("booking_time", "")
    equip = booking.get("equipment_name", "")
    ...
    fub = booking.get("user_fub", "")
    bid = booking.get("id", "")
    reviewed_by = booking.get("reviewed_by", "Lab Staff")
    notes = booking.get("notes", "") or "—"
    note_txt = review_note or booking.get("review_note", "") or "—"

    is_approved = (decision == "approved")
    status_label = "✅ APPROVED" if is_approved else "❌ REJECTED"
    status_word  = "Approved"   if is_approved else "Rejected"
    action_msg = (
        "Your booking has been approved! You may use the equipment at the scheduled time."
        if is_approved else
        "Unfortunately, your booking request has been rejected. "
        "Please contact the laboratory at cancerlab@unb.br if you have questions or wish to reschedule."
    )

    body_user = (
        f"Dear {name},\n\n"
        f"Your equipment booking request has been reviewed.\n\n"
        f"{status_label}\n\n"
        f"{action_msg}\n\n"
        f"━━━━━━ Booking Details ━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"  Booking ID       : #{bid}\n"
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Questions? Contact us at cancerlab@unb.br\n\n"
        f"Best regards,\n"
        f"CancerLab – Laboratory of Molecular Pathology of Cancer\n"
        f"University of Brasília\n"
    ...
    body_lab = (
        f"Booking #{bid} was {status_word.lower()} by {reviewed_by}.\n\n"
        f"━━━━━━ Booking Details ━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"  Booking ID       : #{bid}\n"
        f"  Equipment        : {equip}\n"
        f"  Date             : {date_fmt}\n"
    ...
    ok_user, err_user = _send_email_safe(
        email,
        f"[CancerLab] Booking #{bid} {status_word} – {equip} on {date_fmt}",
        body_user,
    )
    ok_lab, err_lab = _send_email_safe(
        LAB_EMAIL,
        f"[CancerLab] Booking #{bid} {status_word} by {reviewed_by}",
        body_lab,
    )
    # Backup admin
    _send_email_safe(
        ADMIN_EMAIL,
        f"[CancerLab] Booking #{bid} {status_word} – {equip}",
        body_lab,
    )
...
def send_cancellation_email(booking: dict, cancelled_by_label: str = "requester") -> dict:
    """Notify lab staff when a user cancels their own reservation."""
    date_fmt = booking.get("booking_date", "")
    time_fmt = booking.get("booking_time", "")
    equip = booking.get("equipment_name", "")
    name = booking.get("user_name", "")
    ...
    body_lab = (
        ...
        f"  Time           : {time_fmt}\n"
        f"  User           : {name} ({email})\n"
        f"  The time slot is now available.\n"
    )

    ok_user, err_user = _send_email_safe(
        email,
        f"[CancerLab] Booking #{bid} Cancelled – {equip} on {date_fmt}",
        body_user,
    )
    ok_lab, err_lab = _send_email_safe(
        LAB_EMAIL,
        f"[CancerLab] Booking #{bid} Cancelled by {cancelled_by_label}",
        body_lab,
    )
    _send_email_safe(ADMIN_EMAIL, f"[CancerLab] Booking #{bid} Cancelled", body_lab)

    return {"user_sent": ok_user, "user_error": err_user, "lab_sent": ok_lab, "lab_error": err_lab}


# ====================== SCHEDULING PAGE (external users) ======================

def _status_badge_html(status: str) -> str:
    """Returns a styled HTML badge for a booking status."""
    s = str(status or "").lower()
    if s in ("approved", "confirmed"):
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#e6f5ee;color:#0d7c4c;font-size:12px;font-weight:800;">✅ Approved</span>'
    elif s == "pending":
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#fef9c3;color:#854d0e;font-size:12px;font-weight:800;">⏳ Pending Approval</span>'
    elif s == "rejected":
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#fee2e2;color:#b91c1c;font-size:12px;font-weight:800;">❌ Rejected</span>'
    elif s == "cancelled":
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#f1f5f9;color:#64748b;font-size:12px;font-weight:800;">🚫 Cancelled</span>'
    return f'<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#f1f5f9;color:#334155;font-size:12px;font-weight:800;">{html.escape(status)}</span>'


def page_schedule_equipment():
    """Full-page scheduling portal for external users, opened from the Equipment page."""
    ...
    :root{
        --sch-blue:#2a4db5;
        --sch-blue-light:#e8eefa;
        --sch-green:#0d7c4c;
        --sch-green-light:#e6f5ee;
        --sch-red:#b91c1c;
        --sch-red-light:#fee2e2;
        --sch-yellow:#854d0e;
        --sch-yellow-light:#fef9c3;
        --sch-ink:#0f172a;
        --sch-muted:#475569;
        --sch-line:#e2e8f0;
        --sch-card:#ffffff;
    ...
    /* ── Page layout ── */
    .sch-page { max-width:760px; margin:0 auto; padding:28px 20px 60px; }

    /* ── Hero ── */
    .sch-hero { text-align:center; margin-bottom:32px; }
    .sch-hero-icon { font-size:3rem; margin-bottom:8px; }
    .sch-hero h1 { font-size:clamp(22px,3vw,30px); font-weight:900; color:var(--sch-ink); margin:0 0 6px; }
    .sch-hero p  { color:var(--sch-muted); font-size:15px; margin:0; }
    ...
    .sch-card-title {
        ...
        color:var(--sch-ink); margin:0 0 16px;
        display:flex; align-items:center; gap:8px;
        line-height:1.3;
    }
    .sch-back-btn { margin-bottom:16px; }

    /* ── Booking row (user-facing) ── */
    .sch-booking-row {
        border:1px solid var(--sch-line);
        border-radius:12px;
        padding:14px 16px;
    ...
    .sch-booking-row:last-child { margin-bottom:0; }
    .sch-booking-equip {
        font-weight:800;
        color:var(--sch-ink);
        font-size:15px;
    ...
    .sch-calendar-row:last-child { margin-bottom:0; }
    .sch-cal-equip { font-weight:800; color:#1d4ed8; font-size:14px; }
    .sch-cal-meta  { color:var(--sch-muted); font-size:13px; margin-top:2px; }

    /* ── Info/notice boxes ── */
    .sch-pending-info {
        background:#fffbeb; border:1px solid #fcd34d; border-radius:12px;
        padding:14px 16px; margin:12px 0; font-size:14px; color:#78350f;
        line-height:1.6;
    }
    .sch-section-divider {
        border:none; border-top:1px solid var(--sch-line);
        margin:16px 0 14px;
    }

    /* ── Mobile responsiveness ── */
    @media (max-width: 600px) {
        .sch-page { padding:16px 12px 40px; }
        .sch-card { padding:18px 14px 14px; border-radius:12px; }
        .sch-booking-row { padding:12px 12px; }
        .sch-booking-equip { font-size:14px; }
        .sch-booking-meta { font-size:12px; }
        .sch-hero h1 { font-size:20px; }
        .sch-card-title { font-size:15px; }
    }
    </style>
    """, unsafe_allow_html=True)

    st.markdown('<div class="sch-page">', unsafe_allow_html=True)

    # Botão Voltar
    st.markdown('<div class="sch-back-btn">', unsafe_allow_html=True)
    if st.button("← Back to Equipment", key="sch_back"):
        st.session_state.pop("sch_view", None)
        st.session_state.pop("ext_user", None)
        st.session_state.pop("sch_dash_tab", None)
        st.session_state.pop("sch_auth_tab", None)
    ...
    <div class="sch-hero">
        ...
        <h1>Lab Equipment Scheduling</h1>
        <p>CancerLab · University of Brasília</p>
    </div>
    """, unsafe_allow_html=True)

    # ── Session init ──
    if "ext_user" not in st.session_state:
        st.session_state.ext_user = None
    if "sch_auth_tab" not in st.session_state:
        st.session_state.sch_auth_tab = "login"
    # Sync sch_view with login state on every run
    if st.session_state.ext_user:
        if st.session_state.get("sch_view") != "dashboard":
            st.session_state.sch_view = "dashboard"
    else:
        if st.session_state.get("sch_view") != "auth":
            st.session_state.sch_view = "auth"
    if "sch_view" not in st.session_state:
        st.session_state.sch_view = "auth"

    # ======== AUTHENTICATION ========
    if st.session_state.sch_view == "auth":

        col_l, col_r = st.columns(2)
        with col_l:
            if st.button("🔑  Sign In", use_container_width=True, key="sch_tab_login",
                         type="primary" if st.session_state.sch_auth_tab == "login" else "secondary"):
                st.session_state.sch_auth_tab = "login"
                st.rerun()
        with col_r:
            if st.button("📝  Create Account", use_container_width=True, key="sch_tab_reg",
                         type="primary" if st.session_state.sch_auth_tab == "register" else "secondary"):
                st.session_state.sch_auth_tab = "register"
                st.rerun()

        st.markdown('<div class="sch-card">', unsafe_allow_html=True)

        if st.session_state.sch_auth_tab == "login":
            st.markdown('<div class="sch-card-title">🔑 Sign In to Your Account</div>', unsafe_allow_html=True)
            with st.form("sch_login_form"):
                email_in = st.text_input("E-mail", placeholder="your@email.com")
            ...
            if sub:
                if not email_in or not pass_in:
                    ...
                    u = autentica_ext_user(email_in, pass_in)
                    if u:
                        st.session_state.ext_user = u
                        st.session_state.sch_view = "dashboard"
                        st.rerun()
                    else:
            ...
            st.markdown('<div class="sch-card-title">📝 Create Your External Account</div>', unsafe_allow_html=True)
            with st.form("sch_register_form"):
                r_name  = st.text_input("Full Name *", placeholder="Your full name")
                r_email = st.text_input("E-mail *", placeholder="your@email.com")
                r_pass  = st.text_input("Password *", type="password", placeholder="Minimum 6 characters")
                r_pass2 = st.text_input("Confirm Password *", type="password", placeholder="Repeat your password")
                r_phone = st.text_input("Phone *", placeholder="+55 61 9 0000-0000")
                r_fub   = st.text_input("FUB Registration Number *", placeholder="e.g.: 190012345")
            ...
            if sub_r:
                ...
                elif "@" not in r_email: errors.append("Please enter a valid email address.")
                if not r_pass:           errors.append("Password is required.")
                elif len(r_pass) < 6:    errors.append("Password must be at least 6 characters.")
                if r_pass != r_pass2:    errors.append("Passwords do not match.")
                if not r_phone.strip():  errors.append("Phone number is required.")
                if not r_fub.strip():    errors.append("FUB registration number is required.")

                if errors:
                    for e in errors:
                        st.error(e)
                elif ext_user_email_exists(r_email):
                    st.error("This email is already registered. Please sign in instead.")
                else:
                    try:
                        salvar_ext_user(r_name, r_email, r_pass, r_phone, r_fub)
    ...
    # ======== DASHBOARD ========
    elif st.session_state.sch_view == "dashboard":
        u = st.session_state.ext_user
        if not u:
            st.session_state.sch_view = "auth"
            st.rerun()
            st.stop()
        ...
        if "sch_dash_tab" not in st.session_state:
            st.session_state.sch_dash_tab = "new"

        # Top bar
        col_info, col_out = st.columns([3, 1])
        with col_info:
            st.markdown(f"<div style='font-weight:800;font-size:16px;color:#0f172a;margin-bottom:2px;'>Welcome, {u['full_name']}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='font-size:13px;color:#475569;'>{u['email']} &nbsp;·&nbsp; FUB Registration: {u['fub_registration']}</div>", unsafe_allow_html=True)
        with col_out:
            if st.button("Sign Out", key="sch_signout"):
                st.session_state.ext_user = None
                st.session_state.sch_view = "auth"
                st.session_state.sch_auth_tab = "login"
                st.rerun()
        ...
        # Dashboard tabs: New Booking | My Bookings | Calendar

        c1, c2, c3 = st.columns(3)
        with c1:
            if st.button("📅  New Booking", use_container_width=True, key="sch_new_btn",
                         type="primary" if st.session_state.sch_dash_tab == "new" else "secondary"):
                st.session_state.sch_dash_tab = "new"
                st.rerun()
        with c2:
            if st.button("📋  My Bookings", use_container_width=True, key="sch_my_btn",
                         type="primary" if st.session_state.sch_dash_tab == "history" else "secondary"):
                st.session_state.sch_dash_tab = "history"
                st.rerun()
        with c3:
            if st.button("🗓️  Calendar", use_container_width=True, key="sch_cal_btn",
                         type="primary" if st.session_state.sch_dash_tab == "calendar" else "secondary"):
                st.session_state.sch_dash_tab = "calendar"
                st.rerun()

        # ── ABA NOVA RESERVA ──
        if st.session_state.sch_dash_tab == "new":
            st.markdown('<div class="sch-card">', unsafe_allow_html=True)
            st.markdown('<div class="sch-card-title">📅 Request Equipment Booking</div>', unsafe_allow_html=True)

            # Approval notice
            st.markdown("""
            <div class="sch-pending-info">
                ℹ️ <strong>Approval required:</strong> All booking requests are reviewed by the laboratory team before confirmation.
                You will receive an email once your request has been approved or declined.
            </div>
            """, unsafe_allow_html=True)

            # ── PRÉ-SELEÇÃO (fora do formulário para reatividade) ──
            # Equipment and date are outside the form so changing them triggers
            # an immediate rerun and the available time-slots list updates live.
            from datetime import timezone, timedelta as _td
            _BRT = timezone(_td(hours=-3))   # Brasília Standard Time (UTC-3)
            ...
            equip_sel = st.selectbox(
                "Select Equipment *",
                SCHEDULABLE_EQUIPMENT,
                help="Choose the equipment you want to book.",
                key="sch_equip_sel_outer",
            )
            ...
            date_sel = st.date_input(
                ...
                help="You may select today's date. Already-passed time slots will be excluded automatically.",
                key="sch_date_sel_outer",
            )

            # Build all possible time slots (08:00–14:00, 30-min increments)
            all_time_slots = []
            for _h in range(8, 14):
                for _m in (0, 30):
                    all_time_slots.append(f"{_h:02d}:{_m:02d}")
            all_time_slots.append("14:00")

            # Filter past slots on the current date (with 15-min buffer)
            if date_sel > today_local:
                # Future date → all slots available
                available_slots = list(all_time_slots)
            elif date_sel == today_local:
                current_minutes = now_local.hour * 60 + now_local.minute + 15  # 15-min grace
                available_slots = [
                    t for t in all_time_slots
                    if int(t.split(":")[0]) * 60 + int(t.split(":")[1]) > current_minutes
                ]
            else:
                # Past date — should never happen due to min_value guard
                available_slots = []

            # Also remove already-booked slots from the list
            already_booked = {
                b["booking_time"]
                for b in get_all_bookings()
                if b["equipment_name"] == equip_sel
                and b["booking_date"] == str(date_sel)
                and b["status"] in ("pending", "approved", "confirmed")
            }
            available_slots = [t for t in available_slots if t not in already_booked]

            if not available_slots:
                if date_sel == today_local:
                    st.warning("⚠️ No time slots available for today. All slots have passed or are already booked. Please select a future date.")
                else:
                    st.warning("⚠️ All time slots for this date are already booked. Please select a different date.")

            with st.form("sch_booking_form"):
                if not available_slots:
                    st.info("ℹ️ Select a different date above to see available time slots.")
                    sub_b = st.form_submit_button("Submit Booking Request", use_container_width=True, disabled=True)
                else:
                    time_sel = st.selectbox(
                        "Time Slot *",
                        available_slots,
                        help="Available slots between 08:00 and 14:00 (Brasília time).",
                    )
                    notes_in = st.text_area("Notes (optional)", placeholder="Special requirements or observations...")
                    sub_b = st.form_submit_button("Submit Booking Request", use_container_width=True)

            if available_slots and sub_b:
                date_str = str(date_sel)

                # Backend time validation
                try:
                    _h, _m = int(time_sel.split(":")[0]), int(time_sel.split(":")[1])
                    _minutes = _h * 60 + _m
                    _time_ok = (8 * 60) <= _minutes <= (14 * 60)
                except Exception:
                    _time_ok = False

                # Same-day: re-verify slot hasn't passed (use Brasília local time)
                # _BRT and now_local are already computed above outside the form
                _now_brt = now_local
                _past_time_error = False
                if date_sel == _now_brt.date():
                    current_mins_check = _now_brt.hour * 60 + _now_brt.minute + 15
                    if _minutes <= current_mins_check:
                        _time_ok = False
                        _past_time_error = True
                        st.error("⛔ This time slot has already passed. Please choose a later time.")

                if not _time_ok and not _past_time_error:
                    st.error("⛔ Invalid time slot. Bookings are only allowed between 08:00 and 14:00.")
                elif is_slot_taken(equip_sel, date_str, time_sel):
                    st.error("⚠️ This time slot is already booked. Please choose a different date or time.")
                else:
                    try:
                        bid = salvar_booking(
                            ext_user_id=u["id"],
                            ...
                            user_email=u["email"],
                            user_phone=u["phone"],
                            user_fub=u["fub_registration"],
                            equipment=equip_sel,
                            date_str=date_str,
                            time_str=time_sel,
                            notes=notes_in,
                        )
                        booking_record = get_booking_by_id(bid) or {
                            "id": bid,
                            "equipment_name": equip_sel,
                            "booking_date": date_str,
                            "booking_time": time_sel,
                            "user_name": u["full_name"],
                            "user_email": u["email"],
                            "user_phone": u["phone"],
                            "user_fub": u["fub_registration"],
                        ...
                        email_result = send_booking_emails(booking_record)
                        if email_result.get("user_sent"):
                            st.success(
                                f"✅ Booking request submitted! ID: #{bid}. "
                                f"A confirmation has been sent to {u['email']}. "
                                f"You will receive another email once the lab team reviews your request."
                            )
                        else:
                            st.success(
                                f"✅ Booking request submitted! ID: #{bid}. "
                                f"You will receive an email once the lab team approves or declines your request."
                            )
                            st.warning(
                                f"⚠️ Could not send confirmation email ({email_result.get('user_error', 'unknown')}). "
                                f"Your request has been saved — please contact the lab if needed."
                            )
                        st.session_state.sch_dash_tab = "history"
                        st.rerun()
                    except Exception as ex:
                        st.error(f"Error saving booking request: {ex}")

            st.markdown("</div>", unsafe_allow_html=True)

        # ── MY BOOKINGS TAB ──
        elif st.session_state.sch_dash_tab == "history":
            my_bookings = get_user_bookings(u["id"])

            # ── Helper: format a booking date for display ──
            def _fmt_bk_date(d_str):
                try:
            ...
            # ── Split bookings: active (pending/approved) vs past/closed ──
            from datetime import timezone as _tz, timedelta as _tdelta
            _BRT_tz = _tz(_tdelta(hours=-3))
            _now_brt_naive = datetime.now(_BRT_tz).replace(tzinfo=None)

            active_bk   = []
            past_bk     = []
            for _bk in my_bookings:
                if _bk["status"] in ("pending", "approved", "confirmed"):
                    try:
                        _dt = datetime.strptime(f"{_bk['booking_date']} {_bk['booking_time']}", "%Y-%m-%d %H:%M")
                        if _dt > _now_brt_naive:
                            active_bk.append(_bk)
                        else:
                            past_bk.append(_bk)
                    except Exception:
                        active_bk.append(_bk)
            ...
            # Sort: active upcoming first (soonest first), past most-recent first
            active_bk.sort(key=lambda b: (b.get("booking_date",""), b.get("booking_time","")))
            past_bk.sort(key=lambda b: (b.get("booking_date",""), b.get("booking_time","")), reverse=True)

            n_active = len(active_bk)
            n_past   = len(past_bk)
            n_total  = len(my_bookings)

            st.markdown('<div class="sch-card">', unsafe_allow_html=True)
            st.markdown(
                f'<div class="sch-card-title">📋 My Bookings '
                f'<span style="font-weight:400;font-size:14px;color:#475569;">({n_total} total)</span></div>',
                unsafe_allow_html=True
            )

            if not my_bookings:
                st.info("You have no bookings yet. Use the 'New Booking' tab to request equipment usage.")
            else:
                def _render_booking_row(bk, show_cancel=True):
                    badge       = _status_badge_html(bk["status"])
                    notes_txt   = bk.get("notes") or "—"
                    review_note = bk.get("review_note") or ""
                    date_fmt    = _fmt_bk_date(bk.get("booking_date",""))
                    time_fmt    = html.escape(str(bk.get("booking_time","—")))
                    _lab_note_html = (
                        f'<div class="sch-booking-meta" style="margin-top:4px;color:#7c3aed;">' +
                        f'💬 Lab Note: {html.escape(review_note)}</div>'
                        if review_note else ""
                    )
                    _booking_html = (
                        f'<div class="sch-booking-row">' +
                        f'<div class="sch-booking-equip">{html.escape(str(bk["equipment_name"]))}</div>' +
                        f'<div class="sch-booking-meta" style="margin-top:6px;">' +
                        f'📅 {html.escape(date_fmt)} &nbsp;·&nbsp; '
                        f'🕐 {time_fmt} &nbsp;·&nbsp; {badge}</div>' +
                        f'<div class="sch-booking-meta" style="margin-top:4px;">📝 Notes: {html.escape(notes_txt)}</div>' +
                        _lab_note_html +
                        f'<div class="sch-booking-meta" style="margin-top:4px;font-size:11px;opacity:.6;">' +
                        f'Booking ID: \u0023{bk["id"]}</div>' +
                        f'</div>'
                    )
                    st.markdown(_booking_html, unsafe_allow_html=True)

                    if show_cancel and bk["status"] in ("pending", "approved", "confirmed"):
                        bk_date = bk.get("booking_date", "")
                        bk_time = bk.get("booking_time", "00:00")
                        try:
                            from datetime import timezone as _tz2, timedelta as _td2
                            bk_dt   = datetime.strptime(f"{bk_date} {bk_time}", "%Y-%m-%d %H:%M")
                            can_cancel = bk_dt > _now_brt_naive
                        except Exception:
                            can_cancel = True
                        if can_cancel:
                            if st.button(f"🚫 Cancel Booking \u0023{bk['id']}", key=f"sch_user_cancel_{bk['id']}"):
                                cancel_booking(bk["id"], cancelled_by=f"user:{u['full_name']}")
                                bk_full = get_booking_by_id(bk["id"]) or bk
                                send_cancellation_email(bk_full, cancelled_by_label=u["full_name"])
                                st.success(f"Booking \u0023{bk['id']} cancelled successfully. The time slot is now available for others.")
                                st.rerun()

                # ── Active / Upcoming bookings ──
                if active_bk:
                    st.markdown(
                        f'<div style="font-size:13px;font-weight:700;color:#2a4db5;margin-bottom:8px;">' +
                        f'⏳ Upcoming &amp; Active ({n_active})</div>',
                        unsafe_allow_html=True
                    )
                    for bk in active_bk:
                        _render_booking_row(bk, show_cancel=True)
                else:
                    st.info("No upcoming or active bookings.")

                # ── Past / Closed bookings ──
                if past_bk:
                    st.markdown(
                        '<hr class="sch-section-divider">',
                        unsafe_allow_html=True
                    )
                    st.markdown(
                        f'<div style="font-size:13px;font-weight:700;color:#475569;margin-bottom:8px;">' +
                        f'🗂️ Past &amp; Closed ({n_past})</div>',
                        unsafe_allow_html=True
                    )
                    for bk in past_bk:
                        _render_booking_row(bk, show_cancel=False)

            st.markdown("</div>", unsafe_allow_html=True)

        # ── CALENDÁRIO COMPARTILHADO ──
        elif st.session_state.sch_dash_tab == "calendar":
            upcoming = get_upcoming_approved_bookings()
            st.markdown('<div class="sch-card">', unsafe_allow_html=True)
            st.markdown(
                f'<div class="sch-card-title">🗓️ Equipment Availability Calendar '
                f'<span style="font-weight:400;font-size:14px;color:#475569;">— upcoming approved bookings</span></div>',
                unsafe_allow_html=True
            )
            st.markdown(
                "<p style='font-size:13px;color:#475569;margin-bottom:14px;'>"
                "This calendar shows all upcoming approved bookings. Use it to check equipment availability "
                "before submitting a new request.</p>",
                unsafe_allow_html=True
            )

            if not upcoming:
                st.info("No upcoming approved bookings. All time slots are currently available!")
            else:
                from collections import defaultdict
                by_date: dict = defaultdict(list)
                for bk in upcoming:
                    by_date[bk["booking_date"]].append(bk)

                eq_filter_cal = st.selectbox(
                    "Filter by Equipment",
                    ["All Equipment"] + SCHEDULABLE_EQUIPMENT,
                    key="cal_eq_filter"
                )

                # Mapeamento de dias e meses para português
                ...
                for bk_date in sorted(by_date.keys()):
                    rows = by_date[bk_date]
                    if eq_filter_cal != "All Equipment":
                        rows = [r for r in rows if r["equipment_name"] == eq_filter_cal]
                    if not rows:
                        continue

                    try:
                        dobj = datetime.strptime(bk_date, "%Y-%m-%d")
                        # Use Brasília timezone to check "today"
                        from datetime import timezone, timedelta as _td
                        _today_brt = datetime.now(timezone(_td(hours=-3))).date()
                        day_name = _DAYS_EN[dobj.weekday()]
                        month_name = _MONTHS_EN[dobj.month - 1]
                        date_label = f"{day_name}, {month_name} {dobj.day:02d}, {dobj.year}"
                        is_today = (dobj.date() == _today_brt)
                    except Exception:
                        date_label = bk_date
                        is_today = False

                    today_pill = ' <span style="background:#dbeafe;color:#1d4ed8;padding:1px 8px;border-radius:999px;font-size:11px;font-weight:800;">TODAY</span>' if is_today else ""
                    st.markdown(
                        f'<div style="font-weight:800;font-size:14px;color:#0f172a;margin:12px 0 6px;">'
                        f'📅 {html.escape(date_label)}{today_pill}</div>',
                        unsafe_allow_html=True
                    )

                    for bk in sorted(rows, key=lambda r: r["booking_time"]):
                        requester_display = html.escape(bk.get("user_name", "—"))
                        equip_display = html.escape(bk.get("equipment_name", "—"))
                        time_display = bk.get("booking_time", "—")
                        _bk_status = bk.get("status", "approved")
                        _status_label = (
                            ' &nbsp;<span style="font-size:11px;background:#fef9c3;color:#854d0e;' +
                            'padding:1px 6px;border-radius:999px;font-weight:700;">⏳ Pending</span>'
                            if _bk_status == "pending" else ""
                        )
                        st.markdown(
                            f'<div class="sch-calendar-row">' +
                            f'<div class="sch-cal-equip">🔬 {equip_display}{_status_label}</div>' +
                            f'<div class="sch-cal-meta">🕐 {html.escape(str(time_display))} &nbsp;·&nbsp; 👤 {requester_display}</div>' +
                            f'</div>',
                            unsafe_allow_html=True
                        )
...
# ====================== INTERNAL: GERENCIAR AGENDAMENTOS ======================

def _status_badge_html_pt(status: str) -> str:
    """Badge HTML com rótulo em português para uso no painel interno."""
    s = str(status or "").lower()
    if s in ("approved", "confirmed"):
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#e6f5ee;color:#0d7c4c;font-size:12px;font-weight:800;">✅ Aprovado</span>'
    elif s == "pending":
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#fef9c3;color:#854d0e;font-size:12px;font-weight:800;">⏳ Aguardando Aprovação</span>'
    elif s == "rejected":
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#fee2e2;color:#b91c1c;font-size:12px;font-weight:800;">❌ Rejeitado</span>'
    elif s == "cancelled":
        return '<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#f1f5f9;color:#64748b;font-size:12px;font-weight:800;">🚫 Cancelado</span>'
    return f'<span style="display:inline-block;padding:2px 10px;border-radius:999px;background:#f1f5f9;color:#334155;font-size:12px;font-weight:800;">{html.escape(status)}</span>'


def render_manage_bookings():
    """
    Complete internal panel for managing equipment bookings.
    Includes: approval/rejection of pending requests, cancellation, overview, and audit logs.
    Only authenticated internal users have access (called only from the internal panel).
    """
    reviewer_name = (st.session_state.get("usuario_logado") or {}).get("nome", "Usuário Interno")

    def _fmt_date(d_str):
        """Formata YYYY-MM-DD para DD/MM/YYYY (padrão brasileiro)."""
        try:
    ...
    .mb-title { font-weight:900; font-size:18px; color:#0f172a; margin:0 0 16px; }
    .mb-row {
        border:1px solid #e2e8f0; border-radius:10px;
        padding:14px 16px; margin-bottom:8px; background:#fafbfe;
        word-break:break-word;
    }
    .mb-row:last-child { margin-bottom:0; }
    .mb-row-pending { border-left:4px solid #f59e0b !important; background:#fffdf7 !important; }
    .mb-equip { font-weight:800; font-size:15px; color:#0f172a; margin-bottom:2px; }
    .mb-meta  { font-size:13px; color:#475569; margin-top:4px; line-height:1.5; }
    .mb-user  { font-size:13px; color:#1d4ed8; margin-top:4px; font-weight:700; line-height:1.5; }
    .mb-audit-row {
        background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px;
        padding:8px 12px; margin-bottom:4px; font-size:12px; color:#334155;
        word-break:break-word; line-height:1.6;
    }
    .mb-section-title {
        font-weight:800; font-size:15px; color:#0f172a;
        margin:20px 0 10px; padding-bottom:4px;
        border-bottom:2px solid rgba(42,77,181,.2);
    }
    @media (max-width:600px) {
        .mb-equip { font-size:14px; }
        .mb-meta, .mb-user { font-size:12px; }
        .mb-row { padding:10px 12px; }
    }
    </style>
    """, unsafe_allow_html=True)

    st.markdown("## 📋 Gerenciamento de Agendamentos de Equipamentos")
    st.markdown(
        "Painel interno exclusivo para gerenciar todos os agendamentos de equipamentos. "
    ...
    # ── Métricas rápidas ──
    total = len(all_bk)
    n_pending  = len(pending_bk)
    n_approved = len([b for b in all_bk if b["status"] in ("approved", "confirmed")])
    n_rejected = len([b for b in all_bk if b["status"] == "rejected"])
    n_cancelled = len([b for b in all_bk if b["status"] == "cancelled"])

    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("Total", total)
    m2.metric("⏳ Pendentes", n_pending, delta="Requer revisão" if n_pending > 0 else None,
              delta_color="inverse" if n_pending > 0 else "off")
    m3.metric("✅ Aprovados", n_approved)
    m4.metric("❌ Rejeitados", n_rejected)
    m5.metric("🚫 Cancelados", n_cancelled)

    if n_pending > 0:
        st.warning(f"⚠️ {n_pending} solicitação(ões) estão **aguardando aprovação** e precisam de revisão.")

    st.markdown("---")

    # ── Abas: Pendentes | Todos | Log de Auditoria ──
    tab_pending, tab_all, tab_audit = st.tabs([
        f"⏳ Aguardando Aprovação ({n_pending})",
    ...
    with tab_pending:
        st.markdown('<div class="mb-section-title">Solicitações Aguardando sua Decisão</div>', unsafe_allow_html=True)

        if not pending_bk:
            st.success("✅ Nenhuma solicitação pendente. Todas foram revisadas.")
        else:
            for bk in pending_bk:
                notes_txt = bk.get("notes") or "—"
                created_at = str(bk.get("created_at", "—"))[:19]
                st.markdown(
                    f'<div class="mb-row mb-row-pending">' +
                    f'<div class="mb-equip">🔬 {html.escape(bk["equipment_name"])} &nbsp; ⏳ Aguardando Aprovação</div>' +
                    f'<div class="mb-meta">📅 {html.escape(_fmt_date(bk["booking_date"]))} &nbsp;·&nbsp; ' +
                    f'🕐 {html.escape(str(bk["booking_time"]))} &nbsp;·&nbsp; Enviado em: {html.escape(created_at)}</div>' +
                    f'<div class="mb-user">👤 {html.escape(bk["user_name"])} — {html.escape(bk["user_email"])} — ' +
                    f'📞 {html.escape(bk["user_phone"])} — Matrícula FUB: {html.escape(bk["user_fub"])}</div>' +
                    f'<div class="mb-meta">📝 Obs.: {html.escape(notes_txt)} &nbsp;·&nbsp; ' +
                    f'ID: \u0023{bk["id"]}</div>' +
                    f'</div>',
                    unsafe_allow_html=True
                )

                review_note_key = f"rn_{bk['id']}"
                if review_note_key not in st.session_state:
                    st.session_state[review_note_key] = ""

                review_note = st.text_input(
                    f"Observação para #{bk['id']} (opcional)",
                    key=review_note_key,
                    placeholder="ex.: Aprovado – traga seu crachá. / Rejeitado – horário reservado para manutenção."
                )

                col_a, col_r = st.columns(2)
                with col_a:
                    if st.button(f"✅ Aprovar #{bk['id']}", key=f"mb_approve_{bk['id']}", use_container_width=True):
                        updated = approve_booking(bk["id"], reviewed_by=reviewer_name, review_note=review_note)
                        if updated:
                            email_res = send_approval_decision_emails(updated, "approved", review_note)
                            ok_msg = "E-mail de notificação enviado." if email_res.get("user_sent") else f"E-mail não enviado ({email_res.get('user_error', '?')})."
                            st.success(f"✅ Reserva #{bk['id']} aprovada por {reviewer_name}. {ok_msg}")
                        else:
                            st.success(f"✅ Reserva #{bk['id']} aprovada.")
                        st.rerun()

                with col_r:
                    if st.button(f"❌ Rejeitar #{bk['id']}", key=f"mb_reject_{bk['id']}", use_container_width=True):
                        updated = reject_booking(bk["id"], reviewed_by=reviewer_name, review_note=review_note)
                        if updated:
                            email_res = send_approval_decision_emails(updated, "rejected", review_note)
                            ok_msg = "E-mail de notificação enviado." if email_res.get("user_sent") else f"E-mail não enviado ({email_res.get('user_error', '?')})."
                            st.success(f"❌ Reserva #{bk['id']} rejeitada por {reviewer_name}. {ok_msg}")
                        else:
                            st.success(f"❌ Reserva #{bk['id']} rejeitada.")
                        st.rerun()

                st.markdown("---")

    # ── ABA: TODOS OS AGENDAMENTOS ──
    with tab_all:
        st.markdown('<div class="mb-section-title">Todos os Agendamentos</div>', unsafe_allow_html=True)

        col_eq, col_st, col_dt = st.columns(3)
        with col_eq:
            eq_filter = st.selectbox("Filtrar por Equipamento", ["Todos"] + SCHEDULABLE_EQUIPMENT, key="mb_eq_filter")
        with col_st:
            st_filter = st.selectbox(
                "Filtrar por Status",
                ["Todos", "Pendente", "Aprovado", "Rejeitado", "Cancelado"],
                key="mb_st_filter",
            )
        with col_dt:
            date_filter = st.date_input("Filtrar por Data", value=None, key="mb_date_filter")

        _status_display_map = {
            "Pendente": "pending",
            "Aprovado": ["approved", "confirmed"],
            "Rejeitado": "rejected",
            "Cancelado": "cancelled",
        }
        ...
        filtered = all_bk
        if eq_filter != "Todos":
            filtered = [b for b in filtered if b["equipment_name"] == eq_filter]
        if st_filter != "Todos":
            _sv = _status_display_map.get(st_filter, st_filter)
            if isinstance(_sv, list):
                filtered = [b for b in filtered if b["status"] in _sv]
            else:
                filtered = [b for b in filtered if b["status"] == _sv]
        if date_filter:
            filtered = [b for b in filtered if b["booking_date"] == str(date_filter)]

        st.markdown(f"**{len(filtered)} agendamento(s) encontrado(s).**")

        if not filtered:
            st.info("Nenhum agendamento encontrado com os filtros selecionados.")
        else:
            for bk in filtered:
                badge_html = _status_badge_html_pt(bk["status"])
                notes_txt = bk.get("notes") or "—"
                review_note_val = bk.get("review_note") or "—"
                reviewed_by_val = bk.get("reviewed_by") or "—"
                created_at = str(bk.get("created_at", "—"))[:19]
                row_cls = "mb-row mb-row-pending" if bk["status"] == "pending" else "mb-row"
                st.markdown(
                    f'<div class="{row_cls}">' +
                    f'<div class="mb-equip">{html.escape(bk["equipment_name"])} &nbsp; {badge_html}</div>' +
                    f'<div class="mb-meta">📅 {html.escape(_fmt_date(bk["booking_date"]))} &nbsp;·&nbsp; ' +
                    f'🕐 {html.escape(str(bk["booking_time"]))} &nbsp;·&nbsp; Enviado em: {html.escape(created_at)}</div>' +
                    f'<div class="mb-user">👤 {html.escape(bk["user_name"])} — {html.escape(bk["user_email"])} — ' +
                    f'{html.escape(bk["user_phone"])} — Matrícula FUB: {html.escape(bk["user_fub"])}</div>' +
                    f'<div class="mb-meta">📝 Obs.: {html.escape(notes_txt)} &nbsp;·&nbsp; ' +
                    f'ID: \u0023{bk["id"]}</div>' +
                    f'<div class="mb-meta">🔎 Revisado por: {html.escape(reviewed_by_val)} &nbsp;·&nbsp; ' +
                    f'Obs. do revisor: {html.escape(review_note_val)}</div>' +
                    f'</div>',
                    unsafe_allow_html=True
                )
                ...
                if bk["status"] == "pending":
                    with action_cols[0]:
                        if st.button(f"✅ Aprovar #{bk['id']}", key=f"mb_all_approve_{bk['id']}", use_container_width=True):
                            updated = approve_booking(bk["id"], reviewed_by=reviewer_name)
                            if updated:
                                send_approval_decision_emails(updated, "approved")
                            st.success(f"Reserva #{bk['id']} aprovada.")
                            st.rerun()
                    with action_cols[1]:
                        if st.button(f"❌ Rejeitar #{bk['id']}", key=f"mb_all_reject_{bk['id']}", use_container_width=True):
                            updated = reject_booking(bk["id"], reviewed_by=reviewer_name)
                            if updated:
                                send_approval_decision_emails(updated, "rejected")
                            st.success(f"Reserva #{bk['id']} rejeitada.")
                            st.rerun()

                # Cancelar aprovado/confirmado/pendente
                if bk["status"] in ("approved", "confirmed", "pending"):
                    with action_cols[2]:
                        if st.button(f"🚫 Cancelar #{bk['id']}", key=f"mb_all_cancel_{bk['id']}", use_container_width=True):
                            cancel_booking(bk["id"], cancelled_by=f"internal:{reviewer_name}")
                            bk_full = get_booking_by_id(bk["id"]) or bk
                            send_cancellation_email(bk_full, cancelled_by_label=f"equipe do laboratório ({reviewer_name})")
                            st.success(f"Reserva #{bk['id']} cancelada. O usuário foi notificado.")
                            st.rerun()

    # ── ABA: LOG DE AUDITORIA ──
    with tab_audit:
        st.markdown('<div class="mb-section-title">Log de Auditoria — Todas as Ações</div>', unsafe_allow_html=True)
        st.caption("Registro completo de todas as ações realizadas no sistema de agendamentos.")
        ...
        if not audit_logs:
            st.info("Nenhuma entrada encontrada no log de auditoria.")
        else:
            action_icons = {
                "submitted": "📩",
                "approved":  "✅",
            ...
            for log in audit_logs[:200]:   # limit to 200 most recent
                icon = action_icons.get(str(log.get("action", "")).lower(), "📋")
                ts = str(log.get("created_at", "—"))[:19]
                action_key = str(log.get("action", "?")).lower()
                action_label = _action_labels_pt.get(action_key, action_key.upper())
                performed_by = str(log.get("performed_by", "?"))
                note = str(log.get("note", "")) or "—"
                bid_val = log.get("booking_id", "?")
                st.markdown(
                    f'<div class="mb-audit-row">' +
                    f'{icon} <strong>{html.escape(action_label)}</strong>' +
                    f' &nbsp;·&nbsp; Reserva \u0023{html.escape(str(bid_val))}' +
                    f' &nbsp;·&nbsp; Por: {html.escape(performed_by)}' +
                    f' &nbsp;·&nbsp; {html.escape(ts)}' +
                    f'<br><span style="color:#64748b;">Obs.: {html.escape(note)}</span>' +
                    f'</div>',
                    unsafe_allow_html=True
                )

            if len(audit_logs) > 200:
                st.caption(f"Exibindo 200 de {len(audit_logs)} entradas no log de auditoria.")
...
def salvar_usuario(usuario):
    ...
    try:
        ...
        raise ValueError(f"CPF já cadastrado ou dado duplicado no banco: {e}")

    except Exception as e:
        raise RuntimeError(f"Erro ao salvar usuário no PostgreSQL: {e}")
...
def usuario_existe(CPF):
    ...
    with engine.connect() as conn:
        result = conn.execute(
            select(usuarios_table).where(usuarios_table.c.CPF == CPF)
        )
...
AUTH_PAGES = {"login", "criar_conta", "reset_request", "reset"}
APP_PAGES = {"index", "biopsia", "clinicos"}
PUBLIC_PAGES = {"home", "research", "publications", "people", "partners", "positions", "equipment", "news", "schedule_equipment", "equipment_pub"}

# Inicializa sessão antes do roteamento
if "page" not in st.session_state:
    if active_nav in AUTH_PAGES or active_nav in APP_PAGES:
        st.session_state.page = active_nav
    else:
...
# Só sincroniza URL -> sessão em casos seguros.
# IMPORTANTE: não deixe ?page=login sobrescrever criar_conta durante rerun de input.
if active_nav in {"reset_request", "reset"}:
    st.session_state.page = active_nav

elif active_nav == "criar_conta":
    st.session_state.page = "criar_conta"

elif (
    active_nav == "login"
    and st.session_state.usuario_logado is None
):
...
elif active_nav in APP_PAGES and st.session_state.usuario_logado is not None:
    st.session_state.page = active_nav

is_internal = (
    st.session_state.usuario_logado is not None
    or st.session_state.get("page") in AUTH_PAGES
    or st.session_state.get("page") in APP_PAGES
)

# Routing for scheduling pages (public, no login needed)
_cur_page = st.session_state.get("page", "home")

if _cur_page == "schedule_equipment":
    page_schedule_equipment()
    st.stop()

if _cur_page == "equipment_pub":
    # Redirect equipment_pub -> equipment without looping:
    # Only rerun if the session page wasn't already corrected.
    if st.session_state.get("page") != "equipment":
        st.session_state.page = "equipment"
        st.rerun()

# Só renderiza páginas públicas se o usuário NÃO estiver em login/cadastro/reset/app interno
if not is_internal and active_nav in PUBLIC_PAGES:
    {
        "home": page_home,
...
qp_token = _get_param("token")
if qp_token:
    st.session_state.reset_token = qp_token

# ---- PÁGINA DE LOGIN ----
# ---- ACESSO INTERNO: LOGIN / CADASTRO ----
if st.session_state.page == "login":
    ...
    if st.session_state.auth_view == "login":
        ...
        if st.button("Criar conta", key="btn_mostrar_cadastro"):
            st.session_state.auth_view = "cadastro"
            st.rerun()

    # ===================== CADASTRO =====================
    elif st.session_state.auth_view == "cadastro":
        st.title("Criar nova conta")

        with st.form("form_criar_conta_final", clear_on_submit=False):
            nome = st.text_input("Nome completo", key="cad_nome_final")
            CPF = st.text_input("CPF", key="cad_cpf_final")
            email = st.text_input("E-mail", key="cad_email_final")
            senha = st.text_input("Senha", type="password", key="cad_senha_final")
            perfil = st.radio("Tipo de usuário", ["aluno", "servidor"], key="cad_perfil_final")
        ...
        if registrar:
            try:
                ...
                if not (nome_limpo and CPF_limpo and email_limpo and senha):
                ...
                if usuario_existe(CPF_limpo):
                    ...
                    st.error("A conta não foi encontrada no banco após salvar.")

            except Exception as e:
                st.error("Erro ao criar conta.")
                st.code(str(e))

        if st.button("Voltar para o login", key="btn_voltar_login_final"):
            st.session_state.auth_view = "login"
            st.rerun()
...
# PÁGINA INICIAL
    ...
    # NOTE: st.set_page_config is already called at module level — do not call again.
    ...
    with st.container():
        ...
        st.markdown("")
        st.markdown('<hr style="margin:6px 0 12px;border:none;border-top:1px solid rgba(2,6,23,.07);">', unsafe_allow_html=True)
        st.markdown('<p class="cl-title" style="margin-bottom:4px;">Agendamento de Equipamentos</p>', unsafe_allow_html=True)
        st.markdown('<p class="cl-muted">Visualize, filtre, aprove, rejeite e cancele todos os agendamentos de equipamentos feitos por usuários externos.</p>', unsafe_allow_html=True)
        if st.button("📋 Gerenciar Agendamentos", use_container_width=True, key="go_manage_bookings"):
            st.session_state.internal_sub = "manage_bookings"
            st.rerun()
    ...
    # ---- Render Manage Bookings if selected ----
    if st.session_state.get("internal_sub") == "manage_bookings":
        if st.button("← Voltar ao Painel", key="mb_back_btn"):
            st.session_state.pop("internal_sub", None)
            st.rerun()
    ...
    def painel_agenda_bxl(file_path="Entrada e Acompanhamento de Pacientes Biópsia Líquida CCR.xlsx"):
        """
        PAINEL AGENDA BxL — refeito para a planilha atual
    
        REGRAS:
        - Só entra na agenda se houver data válida E NÃO houver "(coletado)"
        - ATRASADA = data válida < hoje
        - PENDENTE/FUTURA = data válida >= hoje
        - "NC", "SP", "F", vazio, "sem cirurgia", "sem tumor" etc. são ignorados
        - IDs com:
            * "exc"   -> EVENTO
            * "saiu"  -> EVENTO
            * "+"     -> FALECIMENTO
          não aparecem na agenda nem no WhatsApp
        - WhatsApp: coletas atrasadas + coletas futuras de hoje até +30 dias
        - Registro persistente na aba REGISTRO_COLETAS
        """
        ...
        FILL_REG = PatternFill("solid", fgColor="C6EFCE")
        FILL_LATE = PatternFill("solid", fgColor="FFEB9C")
        ...
        BLOCKED_TEXTS_CONTAINS = [
            "nao coletado", "não coletado",
            "cancelado", "sem cirurgia", "sem tumor"
        ]
        ...
        def _classify_segmento(raw):
            """
            Normaliza a coluna Segmento para filtros consistentes.
    
            Regras práticas para a planilha atual:
            - variações com cólon/colon, sigmoide, ascendente, descendente, transverso ou ceco -> CÓLON
            - variações com reto/retal, sem marcador de cólon acima -> RETO
            - valores vazios ou não reconhecidos -> INDEFINIDO
    
            Observação: entradas como "cólon transição retossigmoide" são classificadas como CÓLON,
            porque a célula declara cólon. Isso evita que "retossigmoide" jogue esses casos para RETO.
            """
            s = _norm(raw)
            ...
            colon_markers = [
                "colon", "colico", "colica", "ceco", "cecal",
                "sigmoide", "ascendente", "descendente", "transverso",
                "angulo esplenico", "angulo hepatico"
            ]
        ...
        def _open_workbook_safe(path, data_only=True):
            if not os.path.exists(path):
                raise FileNotFoundError(f"Arquivo não encontrado: {path}")
    
            if not zipfile.is_zipfile(path):
                raise ValueError(f"O arquivo não está em formato .xlsx válido: {path}")
    
            tmp_dir = tempfile.gettempdir()
            tmp_copy = os.path.join(
                tmp_dir,
                f"tmp_bxl_{abs(hash(path))}_{'data' if data_only else 'edit'}.xlsx"
            )
            shutil.copy2(path, tmp_copy)
            return openpyxl.load_workbook(tmp_copy, data_only=data_only)
    
        def _save_workbook_atomic(wb, path):
            fd, temp_out = tempfile.mkstemp(suffix=".xlsx")
            os.close(fd)
            try:
                wb.save(temp_out)
        ...
        def _classify_patient_from_id(raw_id):
            ...
            if "+" in raw:
                return {
                    "PatientStatus": "EVENTO",
                    "EventType": "FALECIMENTO",
                    "IsActiveStudy": False,
                    "IsDeath": True,
                    "IsEvent": True,
                }
            ...
            if "exc" in low:
                return {
                    "PatientStatus": "EVENTO",
                    "EventType": "EVENTO NÃO ESPECIFICADO",
                    "IsActiveStudy": False,
                    "IsDeath": False,
                    "IsEvent": True,
                }
        ...
        def _find_main_columns(headers):
            ...
            for idx, h in enumerate(headers, start=1):
                ...
                if cols["id"] is None and (
                    ("id" in nh and "paciente" in nh) or ("cbl" in nh)
                ):
                    cols["id"] = idx
                ...
                if cols["obito"] is None and any(k in nh for k in ["obito", "óbito", "falecimento", "data obito", "data falecimento"]):
                    cols["obito"] = idx
        ...
        def _extract_first_date_from_string(s, today_):
            ...
            low = _norm(s0)
            if low in BLOCKED_TEXTS_EXACT:
                return None
            if any(x in low for x in BLOCKED_TEXTS_CONTAINS):
                return None
        ...
        def _status_map(df_reg):
            ...
            for _, r in df2.iterrows():
                ...
                if cbl and col and diso:
                    m[f"{cbl}||{col}||{diso}"] = stt
    
            return m
    
        def _apply_cell_mark(ws, cell_ref, d, status, motivo, obs):
            if not cell_ref:
                raise ValueError("cell_ref vazio.")
    
            cell = ws[cell_ref]
            stt = str(status or "").strip().upper()
    
            if stt == "REGISTRADA":
                cell.value = f"{d.strftime('%d/%m/%y')} (coletado)"
                cell.number_format = "@"
                cell.fill = FILL_REG
                cell.font = FONT_BOLD
            elif stt == "ATRASADA":
        ...
        def _save_status_to_excel(cbl, coleta_short, dd, status, motivo, obs, sheet_name, cell_ref):
            wb = _open_workbook_safe(file_path, data_only=False)
        ...
        # LEITURA PRINCIPAL
        # =========================================================
        def _load_events_and_patients(sheet_name):
            wb = _open_workbook_safe(file_path, data_only=True)
            ...
            for r in range(2, ws.max_row + 1):
                ...
                obito_date = None
                if obito_col:
                    obito_raw = ws.cell(row=r, column=obito_col).value
                    obito_date = _extract_date(obito_raw, today_, epoch)
    
                valid_dates_in_row = []
                for c in event_cols:
                    raw_any = ws.cell(row=r, column=c).value
                    d_any = _extract_date(raw_any, today_, epoch)
                    if d_any:
                        valid_dates_in_row.append(d_any)
    
                fallback_event_date = max(valid_dates_in_row) if valid_dates_in_row else None
                event_date = obito_date if obito_date else fallback_event_date
                ...
                patient_rows.append({
                    ...
                    "IsEvent": pclass["IsEvent"],
                    "IsDeath": pclass["IsDeath"],
                ...
                for bc in event_cols:
                    ...
                    if raw is None or raw_str == "":
                        continue
    
                    if _should_ignore_cell(raw_str):
                        continue
    
                    collected_tag = _has_collected_tag(raw_str)
                    if collected_tag:
                        continue
                    ...
                    events.append({
                        "CBL": cbl,
                        "CBL_RAW": cbl_raw,
                        "IN_STUDY": _is_active_study_id(cbl_raw),
                        "PatientStatus": pclass["PatientStatus"],
                        "EventType": pclass["EventType"],
                        "Segmento": segmento,
                        "SegmentoGrupo": _classify_segmento(segmento),
                        "Coleta_short": coleta_short,
                        "Data": dd,
                        ...
                        "Raw": raw_str,
                        "CollectedTag": collected_tag,
                    })
    
            df_events = pd.DataFrame(events)
            df_pat = pd.DataFrame(patient_rows).drop_duplicates(subset=["CBL", "CBL_RAW"])
    
            wb2 = _open_workbook_safe(file_path, data_only=True)
            df_reg = _read_registro_df(wb2)
            smap = _status_map(df_reg)
    
            if df_events.empty:
                return df_events, df_pat, phones_map, headers, phone_cols, df_reg, smap
    
            df_events = df_events.drop_duplicates(
                subset=["CBL", "Coleta_short", "Data", "Sheet", "Cell"]
            ).reset_index(drop=True)
    
            df_events["KeyStatus"] = df_events.apply(
                lambda r: f"{r['CBL']}||{r['Coleta_short']}||{r['Data'].isoformat()}",
                axis=1
            )
            df_events["Status"] = df_events["KeyStatus"].apply(lambda k: smap.get(k, "PENDENTE")).astype(str).str.upper()
    
            # remove só as realmente registradas
            df_events = df_events[~df_events["CollectedTag"].astype(bool)].copy()
            df_events = df_events[df_events["Status"] != "REGISTRADA"].copy()
        ...
        # EVENTOS
        # =========================================================
        def painel_eventos(df_pat):
            st.divider()
            st.markdown("### Painel de Eventos")
    
            if df_pat is None or df_pat.empty:
                st.info("Sem pacientes encontrados.")
                return
            ...
            c1.metric("Pacientes", total)
            c2.metric("Ativos", ativos)
            c3.metric("Eventos", eventos)
            c4.metric("Falecimentos", falec)
    
            df_evt = dfu[dfu["IsEvent"] == True].copy()
            if df_evt.empty:
                st.info("Nenhum evento detectado nesta aba.")
                return
    
            vc = (
                df_evt["EventType"]
                .replace("", "EVENTO NÃO ESPECIFICADO")
                .fillna("EVENTO NÃO ESPECIFICADO")
                .value_counts()
                .reset_index()
            )
            ...
            st.dataframe(
                df_evt[["CBL", "EventType", "EventDate_str", "SegmentoGrupo", "Segmento", "CBL_RAW"]].rename(columns={
                    "EventType": "Tipo de evento",
                    "EventDate_str": "Data do evento",
                    "SegmentoGrupo": "Grupo",
                    "CBL_RAW": "ID original",
                }),
                use_container_width=True,
        ...
        # =========================================================
        # SEGMENTO — CÓLON x RETO
        # =========================================================
        def painel_segmentos(df_pat):
            st.divider()
            st.markdown("### Distribuição por segmento")
            st.caption("Distribuição Colon ou Reto")
    
            if df_pat is None or df_pat.empty:
                st.info("Sem pacientes para calcular segmento.")
                return
    
            df_seg = df_pat.copy()
            if "SegmentoGrupo" not in df_seg.columns:
                df_seg["SegmentoGrupo"] = df_seg.get("Segmento", "").apply(_classify_segmento)
    
            df_ativos = df_seg[df_seg["IsActiveStudy"] == True].drop_duplicates(subset=["CBL"]).copy()
    
            ordem = ["CÓLON", "RETO", "INDEFINIDO"]
            cont = (
                df_ativos["SegmentoGrupo"]
                .fillna("INDEFINIDO")
                .replace("", "INDEFINIDO")
                .value_counts()
            ...
            c1, c2, c3 = st.columns(3)
            c1.metric("Cólon", int(cont.loc[cont["Segmento"] == "CÓLON", "Pacientes"].iloc[0]))
            c2.metric("Reto", int(cont.loc[cont["Segmento"] == "RETO", "Pacientes"].iloc[0]))
            c3.metric("Indefinido", int(cont.loc[cont["Segmento"] == "INDEFINIDO", "Pacientes"].iloc[0]))
            ...
            try:
                ...
                fig = px.bar(
                    ...
                    y="Pacientes",
                    text="Pacientes",
                    title="Pacientes ativos por segmento",
                    template="plotly_white",
                )
                ...
                st.bar_chart(graf.set_index("Segmento"))
    
            with st.expander("Auditoria da classificação de segmento", expanded=False):
                audit = df_ativos[["CBL", "Segmento", "SegmentoGrupo"]].sort_values(["SegmentoGrupo", "CBL"])
        ...
        def painel_whatsapp_bxl(df_all_events, phones_map, headers, phone_cols):
            def _extract_phones(raw):
                ...
                candidates = re.findall(
                    r"(?:\+?\s*55\s*)?\(?\s*(\d{2})\s*\)?\s*(9?\d{4,5})[-\s]?(\d{4})",
                    s
                )
            ...
            st.caption("Inclui qualquer coleta atrasada e coletas futuras entre hoje e +30 dias.")
            ...
            if "Dias" not in df.columns:
                st.info("Sem dados suficientes para o WhatsApp.")
                return
            ...
            df_src = df[
                (~df["CollectedTag"].astype(bool)) &
                (df["Status"] != "REGISTRADA") &
                (df["Dias_num"] <= 30)
            ].copy()
    
            df_src = df_src.sort_values(["Data", "CBL", "Coleta_short"]).reset_index(drop=True)
    
            if df_src.empty:
                st.info("Nenhuma coleta elegível para WhatsApp: atrasadas ou futuras até +30 dias.")
                return
    
            phone_colnames = []
            if phone_cols and isinstance(headers, list):
                for pc in phone_cols:
                    nm = headers[pc - 1] if (pc - 1) < len(headers) else f"Telefone_{pc}"
                    phone_colnames.append(str(nm).strip() if nm else f"Telefone_{pc}")
    
            if not phone_colnames:
                phone_colnames = ["Telefone"]
    
            wa_scope = "wa_bxl_atrasadas_0_30"
            ...
            ev = df_src[df_src["CBL"].astype(str) == str(sel_cbl)].copy().sort_values(["Data"])
            if ev.empty:
                st.info("Não há eventos elegíveis para este CBL.")
                return
    
            ev["Label"] = ev.apply(
                lambda r: f"{str(r['Coleta_short']).strip()} — {str(r['Data_str']).strip()} — {str(r.get('Status_display', r.get('Status', ''))).strip()} (D{_safe_int(r['Dias_num'])})",
                axis=1
            )
            ...
            if phones_e164:
                display_opts = [f"{_format_phone_br(p)} (→ {e})" for (e, p) in phones_e164]
                ...
                st.warning("Não encontrei telefone válido. Cole manualmente.")
                manual = st.text_input("Telefone manual", value="", key=f"{wa_scope}_manual")
            ...
            dias_evento = _safe_int(row.get("Dias_num", row.get("Dias", 0)))
            status_evento = "ATRASADA" if dias_evento < 0 else "PENDENTE"
            ...
            if status_evento == "ATRASADA":
                atraso_txt = abs(dias_evento)
                if template == "Lembrete padrão":
                    default_msg = (
                        f"Olá! Identificamos que sua coleta {coleta_txt}, prevista para {data_txt}, "
                        f"está em atraso há {atraso_txt} dia(s). Você poderia nos responder por aqui para reagendarmos?"
                    )
                elif template == "Confirmação de presença":
                    default_msg = (
                        f"Olá! Sua coleta {coleta_txt} estava prevista para {data_txt}. "
                        f"Você consegue confirmar se ainda pode realizar a coleta ou prefere reagendar?"
                    )
                elif template == "Solicitar reagendamento":
                    default_msg = (
                        f"Olá! Sobre a coleta {coleta_txt}, que estava prevista para {data_txt}: "
                        f"por favor, me informe sua disponibilidade para reagendarmos."
                    )
                else:
                    default_msg = "Olá! Estou entrando em contato sobre sua coleta pendente. Pode me responder por aqui, por favor?"
            else:
                if template == "Lembrete padrão":
                    default_msg = (
                        f"Olá! Lembrete: sua coleta {coleta_txt} está agendada para {data_txt}. "
                        f"Se precisar reagendar, por favor responda por aqui."
                    )
                elif template == "Confirmação de presença":
                    default_msg = (
                        f"Olá! Você pode confirmar sua presença para a coleta {coleta_txt} em {data_txt}? "
                        f"Se precisar ajustar a data, me avise por aqui."
                    )
                elif template == "Solicitar reagendamento":
                    default_msg = (
                        f"Olá! Sobre a coleta {coleta_txt} prevista para {data_txt}: você precisa reagendar? "
                        f"Se sim, me informe sua disponibilidade."
                    )
                else:
                    default_msg = "Olá! Estou entrando em contato sobre sua coleta. Pode me responder por aqui, por favor?"
    
            sig = f"{sel_cbl}||{sel_label}||{template}||{status_evento}"
        ...
        try:
            wb0 = _open_workbook_safe(file_path, data_only=True)
        ...
        hide_events = st.sidebar.checkbox("Ocultar eventos/falecimentos", value=True, key="bxl_hide_events")
    
        df_all, df_pat, phones_map, headers, phone_cols, df_reg, smap = _load_events_and_patients(sheet)
    
        st.sidebar.markdown("### Recorte por segmento/paciente")
        segmento_options = ["Todos", "CÓLON", "RETO", "INDEFINIDO"]
        segmento_sel = st.sidebar.selectbox("Segmento", options=segmento_options, index=0, key="bxl_segmento_sel")
        ...
        paciente_sel = st.sidebar.selectbox("Paciente específico", options=pacientes_options, index=0, key="bxl_paciente_sel")
    
        painel_eventos(df_pat)
        painel_segmentos(df_pat)
    
        if df_all.empty:
            st.warning("Nenhuma coleta pendente encontrada nesta aba.")
            return
    
        df = df_all.copy()
    
        if "SegmentoGrupo" not in df.columns:
            df["SegmentoGrupo"] = df.get("Segmento", "").apply(_classify_segmento)
        ...
        if df.empty:
            st.info("Após os filtros, não restaram coletas.")
            return
        ...
        def _status_display(r):
            ...
            collected_tag = bool(r.get("CollectedTag", False))
            dias = pd.to_numeric(r.get("Dias", np.nan), errors="coerce")
    
            if collected_tag or stt == "REGISTRADA":
                return "REGISTRADA"
        ...
        pend = df[
            (~df["CollectedTag"].astype(bool)) &
            (df["Status_display"] != "REGISTRADA")
        ].copy()
    
        pend = pend.sort_values(["Data", "CBL", "Coleta_short"]).reset_index(drop=True)
    
        atras = pend[pend["Dias_num"] < 0].copy().sort_values(["Data", "CBL", "Coleta_short"]).reset_index(drop=True)
        fut = pend[pend["Dias_num"] >= 0].copy().sort_values(["Data", "CBL", "Coleta_short"]).reset_index(drop=True)
    
        fut_0_7 = fut[(fut["Dias_num"] >= 0) & (fut["Dias_num"] <= 7)].copy()
        fut_8_14 = fut[(fut["Dias_num"] >= 8) & (fut["Dias_num"] <= 14)].copy()
        fut_15_30 = fut[(fut["Dias_num"] >= 15) & (fut["Dias_num"] <= 30)].copy()
        fut_31mais = fut[fut["Dias_num"] >= 31].copy()
    
        # =========================================================
        # MÉTRICAS
        # =========================================================
        st.markdown("### Visão geral")
        st.caption(f"Filtro aplicado: Segmento = {segmento_sel} | Paciente = {paciente_sel}")
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Pendentes totais", int(len(pend)))
        m2.metric("Atrasadas", int(len(atras)))
        m3.metric("0–30 dias", int(len(fut[(fut['Dias_num'] >= 0) & (fut['Dias_num'] <= 30)])))
        m4.metric("31+ dias", int(len(fut_31mais)))
    
        SHOW_COLS = ["CBL", "SegmentoGrupo", "Segmento", "Coleta_show", "Data_str", "Dias_num", "Status_display", "Cell"]
    
        tabP, tabA, tab1, tab2, tab3, tab4 = st.tabs([
            "Todas pendentes",
            "Atrasadas",
            "0–7 dias",
            "8–14 dias",
        ...
        with tabA:
            if atras.empty:
                st.info("Nenhuma coleta atrasada.")
            else:
                st.dataframe(
                    atras[SHOW_COLS].rename(columns={
                        "SegmentoGrupo": "Grupo",
                        "Coleta_show": "Coleta",
                        "Status_display": "Status",
                        "Dias_num": "Dias",
        ...
        with tab1:
            if fut_0_7.empty:
                ...
                st.dataframe(
                    fut_0_7[SHOW_COLS].rename(columns={
                        "SegmentoGrupo": "Grupo",
                        "Coleta_show": "Coleta",
                        "Status_display": "Status",
                        "Dias_num": "Dias",
        ...
        with tab2:
            if fut_8_14.empty:
                ...
                st.dataframe(
                    fut_8_14[SHOW_COLS].rename(columns={
                        "SegmentoGrupo": "Grupo",
                        "Coleta_show": "Coleta",
                        "Status_display": "Status",
                        "Dias_num": "Dias",
        ...
        with tab3:
            if fut_15_30.empty:
                ...
                st.dataframe(
                    fut_15_30[SHOW_COLS].rename(columns={
                        "SegmentoGrupo": "Grupo",
                        "Coleta_show": "Coleta",
                        "Status_display": "Status",
                        "Dias_num": "Dias",
        ...
        with tab4:
            if fut_31mais.empty:
                st.info("Nenhuma coleta acima de 30 dias.")
            else:
                st.dataframe(
                    fut_31mais[SHOW_COLS].rename(columns={
                        "SegmentoGrupo": "Grupo",
                        "Coleta_show": "Coleta",
                        "Status_display": "Status",
                        "Dias_num": "Dias",
        ...
        action_df = action_df.sort_values(["Data", "CBL", "Coleta_short"]).reset_index(drop=True)
    
        if action_df.empty:
            st.info("Nada para registrar no recorte atual.")
        else:
            action_df["KeyUI"] = action_df.apply(
                lambda r: f"{r['CBL']}||{r['Coleta_short']}||{r['Data'].isoformat()}||{r.get('Sheet', '')}||{r.get('Cell', '')}",
                axis=1
            )
            action_df["LabelUI"] = action_df.apply(
                lambda r: f"{r['CBL']} — {r['Coleta_short']} — {r['Data_str']} — {r['Status_display']} (D{_safe_int(r['Dias_num'])})",
                axis=1
            )
            ...
            b1, b2, b3 = st.columns([1.0, 1.0, 1.8])
            ...
            with b3:
                st.caption(
                    f"CBL: {sel_row['CBL']} | Coleta: {sel_row['Coleta_short']} | Data: {sel_row['Data_str']} | Dias: {_safe_int(sel_row['Dias_num'])} | Status: {sel_row['Status_display']} | Célula: {sel_row['Cell']}"
                )
    ...
    def _normalize_cbl_text(raw) -> str:
        s = str(raw or "").strip()
        m = re.search(r"\bCBL\s*(\d+)\b", s, flags=re.IGNORECASE)
        if m:
            return f"CBL{int(m.group(1))}"
    ...
    def _is_event_id(raw_id: str) -> bool:
        low = _norm_text(raw_id)
        raw = str(raw_id or "")
    ...
    def _excel_serial_to_ts(x):
        try:
            ...
            if isinstance(x, (int, float)) and -100000 < float(x) < 400000:
                return (pd.to_datetime("1899-12-30") + pd.to_timedelta(int(float(x)), unit="D")).to_pydatetime()
        except Exception:
            pass
        return None
    ...
    # normaliza headers para evitar quebras de linha / espaços estranhos
    df.columns = [_norm_header(c) for c in df.columns]
    st.session_state.df_biopsia.columns = df.columns

    # guarda a aba principal atual
    st.session_state["sheet_biopsia"] = "Página1"
    ...
    colunas_vazias = [
        ...
        if df_visu[col].isna().all() or (df_visu[col].astype(str).str.strip() == "").all()
    ]
    df_visu = df_visu.drop(columns=colunas_vazias, errors="ignore")
    
    st.subheader("Tabela de pacientes")
    
    def _is_blocked_for_date_logic(val) -> bool:
        if val is None:
            return True
        try:
            if pd.isna(val):
        ...
        if s in blocked_exact:
            return True
        if any(x in s for x in blocked_contains):
            return True
    
        return False
    
    def _has_collected_tag(val) -> bool:
        if val is None:
            return False
        return bool(re.search(r"\(\s*coletado\s*\)", str(val), flags=re.IGNORECASE))
    
    def _highlight_excluidos(row):
        id_col = df_visu.columns[0]
    ...
    def _cell_style(val):
        # não tenta estilizar lixo textual
        if _is_blocked_for_date_logic(val):
            return ""
    
        raw = str(val or "").strip().lower()
        d = _parse_date_any(val)
        is_auto = _is_auto_cell(val)
        is_collected = _has_collected_tag(val)
        today = datetime.now().date()
    
        # coletado = nunca vermelho/atrasado
        if is_collected:
            return "color: #111827; font-weight: 700;"
    
        # auto = azul, desde que não seja coletado
        if is_auto:
            return "color: #1d4ed8; font-weight: 700;"
    
        # só entra na lógica de futuro/atrasado se houver data válida
        if d:
            dd = d.date()
            if dd > today:
    ...
    # Compatibilidade robusta entre versões do pandas
    try:
        ...
        if hasattr(styler, "map"):
            ...
            # versões antigas/instáveis do pandas podem não ter map/applymap funcionando bem
            raise AttributeError("Styler.map indisponível nesta versão")
    
        st.dataframe(styler, use_container_width=True)
    except Exception:
        # fallback seguro: mostra a tabela sem quebrar a aplicação
    ...
    st.write(f"**Paciente:** {paciente_selecionado}")

    # =================== Automação BxL baseada em cirurgia ===================
    ...
    bxl_cols = [c for c in df.columns if _is_bxl_col(c)]
    ...
    for c in bxl_cols:
        n = _bxl_num(c)
        if 2 <= n <= 8:
            sequencia.append((f"BxL {n}", c, OFFSETS_MESES.get(n)))
    ...
    def _auto_preencher_datas_futuras(idx_patient: int) -> bool:
        ...
        grade = _grade_alvo(row)
        # Only run automation when a base (surgery) date is present — prevents
        # spurious "dirty" marks that would trigger an autosave → rerun loop.
        if not grade:
            return False
    ...
    def _find_row_by_cbl_robusto(ws, cbl_value: str, cbl_col_idx: int | None) -> int | None:
        if not cbl_value or not cbl_col_idx:
            return None

        alvo = _normalize_cbl_text(cbl_value)

        for r in range(2, ws.max_row + 1):
            v = ws.cell(row=r, column=cbl_col_idx).value
            if v is None:
                continue
            atual = _normalize_cbl_text(v)
            if atual == alvo:
                return r

        return None

    def _find_bxl_col_robusto(hmap, col_bxl):
        alvo = _norm_header(col_bxl)
        if alvo in hmap:
            return hmap[alvo]

        alvo_low = alvo.lower()
        for k, v in hmap.items():
            if str(k).strip().lower() == alvo_low:
                return v
        return None
    ...
    bxl_cols_registro = [c for c in df.columns if _is_bxl_col(c)]
    bxl_cols_registro = sorted(bxl_cols_registro, key=_bxl_num)
    ...
    with cols_bxl[0]:
        if st.button("Registrar data de hoje na BxL selecionada", key=f"bxlbtn_{paciente_selecionado}_{col_bxl}_{idx}"):
            ...
            excel_row = _find_row_by_cbl_robusto(ws, str(paciente_selecionado).strip(), cbl_col_idx)
            if excel_row is None:
                excel_row = int(idx) + 2

            excel_col = _find_bxl_col_robusto(hmap, col_bxl)

            if excel_col is None:
                st.error("Não consegui mapear a coluna BxL no Excel.")
            else:
                cell_ref = get_column_letter(excel_col) + str(excel_row)

                rec = {
                    "CBL": _normalize_cbl_text(str(paciente_selecionado).strip()),
    ...
    with st.expander("Opções avançadas de salvamento / backup", expanded=False):
        st.caption(f"Arquivo: `{FILE_PATH}`")
    ...
    def _limpar_id_cbl_para_exclusao(raw_id: str) -> str:
        """
        Recebe qualquer variação do identificador e retorna apenas CBLn.
        Exemplos:
        - 'CBL3 (exc)' -> 'CBL3'
        - '⛔ CBL5 🚪' -> 'CBL5'
        - 'CBL 12 (+) exc?' -> 'CBL12'
    ...
    def _id_ja_tem_falecimento(raw_id: str) -> bool:
        """
        Detecta se o identificador já tem marca de falecimento.
        Considera '+' em qualquer posição.
        """
        return "+" in str(raw_id or "")
    
    
    def _padronizar_id_exclusao(raw_id: str, motivo: str) -> str:
        """
        Padroniza a retirada do estudo conforme a planilha atual.
    
        Regra:
        - Retirada comum: CBLn (exc)
        - Falecimento: CBLn (+) (exc)
    
        Remove emojis, 'exc' solto, 'saiu', 'excluído', 'exc?' e outros ruídos.
        """
        base = _limpar_id_cbl_para_exclusao(raw_id)
        motivo_norm = _norm_text(motivo)
    ...
    if st.session_state.get(flow_key, False):
        ...
        st.markdown(
            f"<div class='texto-discreto'>"
            f"<b>Identificador atual:</b> {ident_str}<br>"
            f"<b>Nome exibido:</b> {paciente_selecionado}"
            f"</div>",
            unsafe_allow_html=True
        )
        ...
        novo_identificador = _padronizar_id_exclusao(ident_str, motivo_exclusao)
    
        st.markdown(
            f"<div class='texto-discreto'>"
            f"Você está prestes a retirar <b>{ident_str}</b> do estudo por "
            f"<b>{motivo_exclusao}</b> na data de <b>{hoje_txt}</b>.<br><br>"
            f"<b>Novo identificador padronizado:</b> {novo_identificador}"
            f"</div>",
            unsafe_allow_html=True
        )
        ...
        with col_ok:
            if st.button("Confirmar retirada do estudo", key=f"btn_confirmar_exclusao_{idx}"):
                if not confirm_flag:
                    st.error("Marque a opção de confirmação antes de continuar.")
                else:
                    ...
                    # em vez de colocar ⛔, 🚪, ✝ etc., grava o ID como CBLn (exc)
                    # ou CBLn (+) (exc), seguindo o padrão da planilha.
                    df_bio.at[idx, nome_col] = novo_identificador
    
                    st.session_state.df_biopsia = df_bio
                    ...
                    st.success(
                        f"Paciente {ident_str} retirado do estudo por {motivo_exclusao}. "
                        f"Novo identificador: {novo_identificador}"
                    )
                    st.rerun()
    ...
    tcle_path = df.at[idx, "TCLE FOTO"] if "TCLE FOTO" in df.columns else ""
            ...
            if tcle_foto is not None:
                if st.button("Registrar novo TCLE", key=f"reg_tcle_biopsia_{idx}"):
                    caminho = os.path.join(
                        PASTA_TCLE,
                        f"tcle_{_normalize_cbl_text(paciente_selecionado)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                    )
    ...
    with st.form("novo_paciente_form_biopsia"):
        ...
        if novo_tcle_foto is not None:
            ...
            novo_caminho = os.path.join(
                PASTA_TCLE,
                f"novo_tcle_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
            )
        ...
        if submitted:
            if not novo_nome:
                ...
                for c in bxl_cols:
                    if _bxl_num(c) == 2:
                        d2 = _parse_date_any(novos_campos.get(c))
                        break

                if d2:
                    ordenadas = sorted([c for c in df.columns if _is_bxl_col(c)], key=_bxl_num)
                    col_bxl2 = None
                    for c in ordenadas:
                        if _bxl_num(c) == 2:
                            col_bxl2 = c
                    ...
                    if col_bxl2 and col_bxl2 in ordenadas:
                        i2 = ordenadas.index(col_bxl2)
                ...
                if "data TCLE" in df.columns:
                    novos_campos["data TCLE"] = datetime.now().strftime("%d/%m/%y %H:%M")
    ...
    st.dataframe(df.head(100))    # --- CAMPOS PRINCIPAIS: ajuste se os nomes das colunas forem diferentes na sua planilha! ---
