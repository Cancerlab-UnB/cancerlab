...
def salvar_booking(ext_user_id, user_name, user_email, user_phone, user_fub, equipment, date_str, time_str, notes=""):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(booking_audit_table.insert().values(
            ...
            note='Reservation submitted by external user.',
            created_at=datetime.utcnow(),
        ))
        return bid
...
def get_upcoming_approved_bookings() -> list:
    """Returns approved/confirmed bookings from today onward for the shared calendar."""
    garantir_tabelas_schedule()
    ...
    with engine.connect() as conn:
        res = conn.execute(
            select(bookings_table)
            .where(
                bookings_table.c.status.in_(['approved', 'confirmed']) &
                (bookings_table.c.booking_date >= today_str)
            )
...
def send_booking_emails(booking: dict) -> dict:
    ...
    body_user = (
        f"Dear {name},\n\n"
        f"Your equipment reservation request has been received at CancerLab – University of Brasília.\n\n"
        f"⏳ STATUS: PENDING APPROVAL\n"
        f"An authorized lab staff member will review your request and you will receive\n"
        f"an email notification once a decision has been made.\n\n"
        f"━━━━━━ Reservation Request Details ━━━━━━\n"
        f"  Booking ID   : #{bid}\n"
        f"  Equipment    : {equip}\n"
        f"  Date         : {date_fmt}\n"
        f"  Time         : {time_fmt}\n"
        ...
        f"If you have questions, please contact the lab at cancerlab@unb.br.\n\n"
        f"Best regards,\n"
    ...
    body_lab = (
        f"⚠️ NEW RESERVATION REQUEST — Awaiting Your Review\n\n"
        f"A new equipment reservation request has been submitted and requires approval.\n\n"
        f"━━━━━━ Booking Details ━━━━━━\n"
        f"  Booking ID   : #{bid}\n"
        f"  Equipment    : {equip}\n"
        f"  Date         : {date_fmt}\n"
        ...
        f"  User Type    : {user_type}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Please log in to the CancerLab internal panel to approve or reject this request."
    )

    # Send to requester
    ok_user, err_user = _send_email_safe(
        email,
        f"[CancerLab] Reservation Request Received – #{bid} ({equip} on {date_fmt})",
        body_user,
    )
    # Send to lab (cancerlab@unb.br)
    ok_lab, err_lab = _send_email_safe(
        LAB_EMAIL,
        f"[CancerLab] ⚠️ New Reservation #{bid} Pending Approval – {equip} on {date_fmt}",
        body_lab,
    )
...
def send_approval_decision_emails(booking: dict, decision: str, review_note: str = "") -> dict:
    ...
    action_msg = (
        "Your reservation has been approved! You may proceed to use the equipment at the scheduled time."
        if is_approved else
        "Unfortunately, your reservation request has been rejected. "
        "Please contact the lab at cancerlab@unb.br if you have questions or wish to reschedule."
    )

    body_user = (
        f"Dear {name},\n\n"
        f"Your equipment reservation request has been reviewed.\n\n"
        f"{status_label}\n\n"
        f"{action_msg}\n\n"
    ...
    body_lab = (
        f"Reservation #{bid} has been {status_word.lower()} by {reviewed_by}.\n\n"
        f"━━━━━━ Booking Details ━━━━━━\n"
        f"  Booking ID   : #{bid}\n"
        f"  Equipment    : {equip}\n"
        f"  Date         : {date_fmt}\n"
    ...
    ok_user, err_user = _send_email_safe(
        email,
        f"[CancerLab] Reservation #{bid} {status_word} – {equip} on {date_fmt}",
        body_user,
    )
    ok_lab, err_lab = _send_email_safe(
        LAB_EMAIL,
...
def send_cancellation_email(booking: dict, cancelled_by_label: str = "requester") -> dict:
    ...
    body_lab = (
        f"Reservation #{bid} was cancelled by the {cancelled_by_label}.\n\n"
        f"  Equipment : {equip}\n"
        f"  Date      : {date_fmt}\n"
        f"  Time      : {time_fmt}\n"
        f"  User      : {name} ({email})\n"
        f"  Time slot is now available.\n"
    )

    ok_user, err_user = _send_email_safe(
        email,
        f"[CancerLab] Reservation #{bid} Cancelled – {equip} on {date_fmt}",
        body_user,
    )
    ok_lab, err_lab = _send_email_safe(
        LAB_EMAIL,
        f"[CancerLab] Reservation #{bid} Cancelled by {cancelled_by_label}",
        body_lab,
    )
...
def _status_badge_html(status: str) -> str:
    """Return a styled badge HTML string for a booking status."""
...
def page_schedule_equipment():
    ...
    .sch-booking-row {
        border:1px solid var(--sch-line); border-radius:12px;
        padding:14px 16px; margin-bottom:10px; background:#fff;
    }
    .sch-booking-equip { font-weight:800; color:var(--sch-ink); font-size:15px; }
    .sch-booking-meta  { color:var(--sch-muted); font-size:13px; margin-top:4px; }
    .sch-calendar-row {
        border:1px solid var(--sch-line); border-radius:10px;
        padding:10px 14px; margin-bottom:8px; background:#f8faff;
    }
    .sch-cal-equip { font-weight:800; color:#1d4ed8; }
    ...
    if st.session_state.sch_view == "auth":
        ...
        if st.session_state.sch_auth_tab == "login":
            ...
            with st.form("sch_register_form"):
                ...
                r_phone = st.text_input("Phone Number *", placeholder="+55 61 9 0000-0000")
                r_fub   = st.text_input("FUB Registration Number (Matrícula FUB) *", placeholder="e.g. 190012345")
    ...
    # ======== DASHBOARD VIEW ========
        ...
        with col_info:
            ...
            st.markdown(f"<div style='font-size:13px;color:#475569;'>{u['email']} &nbsp;·&nbsp; FUB: {u['fub_registration']}</div>", unsafe_allow_html=True)
        with col_out:
        ...
        if st.session_state.sch_dash_tab == "new":
            st.markdown('<div class="sch-card">', unsafe_allow_html=True)
            st.markdown('<div class="sch-card-title">📅 Request Equipment Reservation</div>', unsafe_allow_html=True)

            # Pending approval notice
            st.markdown("""
            <div class="sch-pending-info">
                ℹ️ <strong>Approval required:</strong> All reservation requests are reviewed by lab staff before confirmation.
            ...
            with st.form("sch_booking_form"):
                equip_sel = st.selectbox(
                    "Select Equipment *",
                    SCHEDULABLE_EQUIPMENT,
                    help="Choose the equipment you wish to reserve."
                )
                # ── SAME-DAY SCHEDULING (Requirement 1) ──
                # Allow today but filter out time slots that have already passed.
                date_sel = st.date_input(
                    "Date *",
                    value=date.today(),
                    min_value=date.today(),   # today allowed
                    help="You may select today's date. Past time slots will be excluded."
                )

                # Build available time slots (08:00–14:00, 30-min increments)
                all_time_slots = []
                for h in range(8, 14):
                    for m in (0, 30):
                ...
                # For same-day: exclude slots that have already passed (add 15-min buffer)
                now_local = datetime.utcnow()  # approximation; UTC used server-side
                if date_sel == date.today():
                    current_minutes = now_local.hour * 60 + now_local.minute + 15  # 15-min buffer
                    available_slots = [
                ...
                if not available_slots:
                    st.warning("⚠️ No time slots available for today. Please select a future date.")
                    sub_b = st.form_submit_button("Confirm Request", use_container_width=True, disabled=True)
                else:
                    time_sel = st.selectbox(
                        "Time Slot *",
                        available_slots,
                        help="Reservations are available between 08:00 and 14:00.",
                    )
                    notes_in = st.text_area("Notes (optional)", placeholder="Any special requirements or observations...")
                    sub_b = st.form_submit_button("Submit Reservation Request", use_container_width=True)
            ...
            if available_slots and sub_b:
                ...
                # Same-day: re-verify slot hasn't passed
                if date_sel == date.today():
                    now_check = datetime.utcnow()
                    current_mins_check = now_check.hour * 60 + now_check.minute + 15
                    if _minutes <= current_mins_check:
                        _time_ok = False
                        st.error("⛔ That time slot has already passed. Please choose a later slot.")

                if not _time_ok:
                    st.error("⛔ Invalid time slot. Reservations are only allowed between 08:00 and 14:00.")
                elif is_slot_taken(equip_sel, date_str, time_sel):
                    st.error("⚠️ This time slot is already reserved. Please choose a different date or time.")
                else:
                    try:
                        bid = salvar_booking(
                            ext_user_id=u["id"],
                        ...
                        if email_result.get("user_sent"):
                            st.success(
                                f"✅ Reservation request submitted! ID: #{bid}. "
                                f"A confirmation has been sent to {u['email']}. "
                                f"You'll receive another email once reviewed by lab staff."
                            )
                        else:
                            st.success(
                                f"✅ Reservation request submitted! ID: #{bid}. "
                                f"You'll receive an email once lab staff approves or rejects it."
                            )
                            st.warning(
                                f"⚠️ Confirmation email could not be sent ({email_result.get('user_error', 'unknown')}). "
                                f"Your request is saved — please contact the lab if needed."
                            )
                        st.session_state.sch_dash_tab = "history"
                        st.rerun()
                    except Exception as ex:
                        st.error(f"Error saving reservation request: {ex}")

            st.markdown("</div>", unsafe_allow_html=True)

        # ── MY BOOKINGS TAB (with Cancellation – Requirement 2) ──
        elif st.session_state.sch_dash_tab == "history":
            my_bookings = get_user_bookings(u["id"])
            st.markdown('<div class="sch-card">', unsafe_allow_html=True)
            st.markdown(
                f'<div class="sch-card-title">📋 My Reservations '
                f'<span style="font-weight:400;font-size:14px;color:#475569;">({len(my_bookings)} total)</span></div>',
                unsafe_allow_html=True
            )

            if not my_bookings:
                st.info("You have no reservations yet. Use the 'New Booking' tab to request equipment time.")
            else:
                for bk in my_bookings:
                    ...
                    st.markdown(f"""
                    <div class="sch-booking-row">
                        <div class="sch-booking-equip">{html.escape(bk['equipment_name'])}</div>
                        <div class="sch-booking-meta">
                            📅 {bk['booking_date']} &nbsp;·&nbsp; 🕐 {bk['booking_time']} &nbsp;·&nbsp; {badge}
                        </div>
                        <div class="sch-booking-meta" style="margin-top:4px;">📝 {html.escape(notes_txt)}</div>
                        {f'<div class="sch-booking-meta" style="margin-top:2px;color:#7c3aed;">💬 Note from lab: {html.escape(review_note)}</div>' if review_note else ''}
                        <div class="sch-booking-meta" style="margin-top:2px;font-size:11px;opacity:.6;">Booking ID: #{bk['id']}</div>
                    </div>
                    """, unsafe_allow_html=True)

                    # Allow cancellation for pending or approved (not cancelled/rejected)
                    if bk["status"] in ("pending", "approved", "confirmed"):
                        # Only allow cancellation if the booking date hasn't fully passed
                        bk_date = bk.get("booking_date", "")
                        bk_time = bk.get("booking_time", "00:00")
                        try:
                            bk_dt = datetime.strptime(f"{bk_date} {bk_time}", "%Y-%m-%d %H:%M")
                            can_cancel = bk_dt > datetime.utcnow()
                        except Exception:
                            can_cancel = True

                        if can_cancel:
                            if st.button(f"🚫 Cancel Reservation #{bk['id']}", key=f"sch_user_cancel_{bk['id']}"):
                                ...
                                st.success(f"Reservation #{bk['id']} has been cancelled. The time slot is now available.")
                                st.rerun()

            st.markdown("</div>", unsafe_allow_html=True)

        # ── SHARED CALENDAR VIEW (Requirement 3) ──
            ...
            st.markdown(
                ...
                f'<span style="font-weight:400;font-size:14px;color:#475569;">— upcoming approved reservations</span></div>',
                unsafe_allow_html=True
            )
            st.markdown(
                "<p style='font-size:13px;color:#475569;margin-bottom:14px;'>"
                "This calendar shows all approved upcoming reservations. Use it to check equipment availability "
                "before submitting a new request.</p>",
                unsafe_allow_html=True
            )

            if not upcoming:
                st.info("No upcoming approved reservations. All time slots are currently available!")
            else:
                ...
                for bk_date in sorted(by_date.keys()):
                    ...
                    try:
                        ...
                        date_label = dobj.strftime("%A, %B %d, %Y")
                        is_today = (dobj.date() == date.today())
                    except Exception:
                        date_label = bk_date
                        is_today = False
                    ...
                    for bk in sorted(rows, key=lambda r: r["booking_time"]):
                        # Respect visibility: show requester name but not email/phone to external users
                        ...
                        st.markdown(f"""
                        <div class="sch-calendar-row">
                            <div class="sch-cal-equip">🔬 {equip_display}</div>
                            <div class="sch-cal-meta">🕐 {time_display} &nbsp;·&nbsp; 👤 {requester_display}</div>
                        </div>
                        """, unsafe_allow_html=True)
...
def render_manage_bookings():
    """
    Painel interno completo para gerenciamento de agendamentos de equipamentos.
    Inclui: aprovação/rejeição de pendentes, cancelamento, visão geral, logs de auditoria.
    Apenas usuários internos autenticados têm acesso (chamado somente do painel interno).
    """
    reviewer_name = (st.session_state.get("usuario_logado") or {}).get("nome", "Internal User")
    ...
    .mb-row-pending { border-left:4px solid #f59e0b !important; }
    .mb-equip { font-weight:800; font-size:15px; color:#0f172a; }
    .mb-meta  { font-size:13px; color:#475569; margin-top:3px; }
    .mb-user  { font-size:13px; color:#1d4ed8; margin-top:3px; font-weight:700; }
    .mb-audit-row { background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px;
    ...
    st.markdown(
        "Painel interno exclusivo para gerenciar todas as reservas de equipamentos. "
    ...
    m2.metric("⏳ Pending", n_pending, delta="Requires review" if n_pending > 0 else None,
              delta_color="inverse" if n_pending > 0 else "off")
    m3.metric("✅ Approved", n_approved)
    m4.metric("❌ Rejected", n_rejected)
    m5.metric("🚫 Cancelled", n_cancelled)

    if n_pending > 0:
        st.warning(f"⚠️ {n_pending} reservation(s) are **pending approval** and require your review.")

    st.markdown("---")

    # ── Tabs: Pending | All | Audit Log ──
    tab_pending, tab_all, tab_audit = st.tabs([
        f"⏳ Pending Approval ({n_pending})",
        "📋 All Reservations",
        "🔍 Audit Log"
    ])
    ...
    with tab_pending:
        st.markdown('<div class="mb-section-title">Reservations Awaiting Your Decision</div>', unsafe_allow_html=True)

        if not pending_bk:
            st.success("✅ No pending reservations. All requests have been reviewed.")
        else:
            for bk in pending_bk:
                notes_txt = bk.get("notes") or "—"
                created_at = str(bk.get("created_at", "—"))[:19]
                st.markdown(f"""
                <div class="mb-row mb-row-pending">
                    <div class="mb-equip">🔬 {html.escape(bk['equipment_name'])} &nbsp; ⏳ Pending Approval</div>
                    <div class="mb-meta">📅 {bk['booking_date']} &nbsp;·&nbsp; 🕐 {bk['booking_time']} &nbsp;·&nbsp; Submitted: {created_at}</div>
                    <div class="mb-user">👤 {html.escape(bk['user_name'])} — {html.escape(bk['user_email'])} — 📞 {html.escape(bk['user_phone'])} — FUB: {html.escape(bk['user_fub'])}</div>
                    <div class="mb-meta">📝 Notes: {html.escape(notes_txt)} &nbsp;·&nbsp; ID: #{bk['id']}</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                review_note = st.text_input(
                    f"Note for #{bk['id']} (optional)",
                    key=review_note_key,
                    placeholder="e.g. Approved – please bring your ID card. / Rejected – slot already taken by maintenance."
                )

                col_a, col_r = st.columns(2)
                with col_a:
                    if st.button(f"✅ Approve #{bk['id']}", key=f"mb_approve_{bk['id']}", use_container_width=True):
                        ...
                        if updated:
                            ...
                            ok_msg = "Email notificações enviados." if email_res.get("user_sent") else f"Email não enviado ({email_res.get('user_error', '?')})."
                ...
                with col_r:
                    if st.button(f"❌ Reject #{bk['id']}", key=f"mb_reject_{bk['id']}", use_container_width=True):
    ...
    with tab_all:
        ...
        with col_eq:
            eq_filter = st.selectbox("Filtrar Equipamento", ["Todos"] + SCHEDULABLE_EQUIPMENT, key="mb_eq_filter")
        with col_st:
        ...
        if not filtered:
            ...
            for bk in filtered:
                badge_html = _status_badge_html(bk["status"])
                notes_txt = bk.get("notes") or "—"
                ...
                st.markdown(f"""
                <div class="{row_cls}">
                    <div class="mb-equip">{html.escape(bk['equipment_name'])} &nbsp; {badge_html}</div>
                    <div class="mb-meta">📅 {bk['booking_date']} &nbsp;·&nbsp; 🕐 {bk['booking_time']} &nbsp;·&nbsp; Submitted: {created_at}</div>
                    <div class="mb-user">👤 {html.escape(bk['user_name'])} — {html.escape(bk['user_email'])} — {html.escape(bk['user_phone'])} — FUB: {html.escape(bk['user_fub'])}</div>
                    <div class="mb-meta">📝 {html.escape(notes_txt)} &nbsp;·&nbsp; ID: #{bk['id']}</div>
                    <div class="mb-meta">🔎 Reviewed by: {html.escape(reviewed_by_val)} &nbsp;·&nbsp; Note: {html.escape(review_note_val)}</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                if bk["status"] in ("approved", "confirmed", "pending"):
                    with action_cols[2]:
                        if st.button(f"🚫 Cancelar #{bk['id']}", key=f"mb_all_cancel_{bk['id']}", use_container_width=True):
                            ...
                            bk_full = get_booking_by_id(bk["id"]) or bk
                            send_cancellation_email(bk_full, cancelled_by_label=f"lab staff ({reviewer_name})")
                            st.success(f"Reserva #{bk['id']} cancelada. Notificação enviada ao usuário.")
                            st.rerun()
    ...
    with tab_audit:
        ...
        if not audit_logs:
            ...
            for log in audit_logs[:200]:   # cap at 200 most recent
                icon = action_icons.get(str(log.get("action", "")).lower(), "📋")
                ts = str(log.get("created_at", "—"))[:19]
                action_label = str(log.get("action", "?")).upper()
                performed_by = str(log.get("performed_by", "?"))
                note = str(log.get("note", "")) or "—"
                bid_val = log.get("booking_id", "?")
                st.markdown(f"""
                <div class="mb-audit-row">
                    {icon} <strong>{action_label}</strong> &nbsp;·&nbsp; Booking #{bid_val}
                    &nbsp;·&nbsp; By: {html.escape(performed_by)}
                    &nbsp;·&nbsp; {ts}
                    <br><span style="color:#64748b;">Note: {html.escape(note)}</span>
                </div>
                """, unsafe_allow_html=True)

            if len(audit_logs) > 200:
                st.caption(f"Showing 200 of {len(audit_logs)} audit entries.")
...
# ---- PÁGINA DE LOGIN ----
# ---- INTERNAL ACCESS: LOGIN / CADASTRO ----
if st.session_state.page == "login":
    ...
    # ===================== CADASTRO =====================
        ...
        with st.form("form_criar_conta_final", clear_on_submit=False):
            ...
            email = st.text_input("Email", key="cad_email_final")
        ...
        if st.button("Voltar para login", key="btn_voltar_login_final"):
            st.session_state.auth_view = "login"
            st.rerun()
...
# PÁGINA INICIAL
    ...
    with st.container():
        ...
        st.markdown('<p class="cl-muted">Visualize, filtre e cancele todos os agendamentos de equipamentos feitos por usuários externos.</p>', unsafe_allow_html=True)
