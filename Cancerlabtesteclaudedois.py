...
def render_manage_bookings():
    ...
    if st_filter != "Todos":
        _status_map = {"confirmado": "confirmed", "cancelado": "cancelled"}
...
if _cur_page == "schedule_equipment":
    show_lab_header(image_relpath="static/imgfront", address_lines=["Laboratory of Molecular Pathology of Cancer", "Faculty of Health Sciences", "University of Brasília", "Brasília – DF – Brazil"])
    render_navbar("equipment")
...
# Só deixa a URL controlar páginas de recuperação de senha.
# NÃO deixe ?page=login sobrescrever o app interno durante reruns.
if qp_page in {"reset_request", "reset"}:
    st.session_state.page = qp_page

# Se o usuário NÃO está logado, a URL pode abrir login.
elif st.session_state.get("usuario_logado") is None and qp_page == "login":
    st.session_state.page = "login"

# Se o usuário JÁ está logado, nunca volte automaticamente para login.
elif st.session_state.get("usuario_logado") is not None and st.session_state.get("page") == "login":
...
page_url = _get_param("page", "login")

if "page" not in st.session_state:
    st.session_state.page = page_url if page_url in valid_pages else "login"
