...
def is_slot_taken(equipment: str, booking_date: str, booking_time: str, exclude_id: int = None) -> bool:
    ...
    with engine.connect() as conn:
        q = select(bookings_table).where(
            ...
            (bookings_table.c.status != 'cancelled')
        )
        if exclude_id:
...
def cancel_booking(booking_id: int):
    ...
    with engine.begin() as conn:
        conn.execute(
            bookings_table.update().where(bookings_table.c.id == booking_id).values(status='cancelled')
        )
...
def send_booking_emails(booking: dict) -> dict:
    """Send confirmation to user AND admin. Returns dict with delivery status."""
    ...
    body_user = (
        f"Dear {name},\n\n"
        f"Your equipment reservation at CancerLab – University of Brasília has been confirmed.\n\n"
        f"━━━━━━ Booking Details ━━━━━━\n"
        f"  Booking ID   : #{bid}\n"
        f"  Equipment    : {equip}\n"
        f"  Date         : {date_fmt}\n"
        ...
        f"  FUB ID       : {fub}\n"
        f"  Phone        : {phone}\n\n"
        f"If you need to cancel or modify your booking, please contact the lab.\n\n"
        f"Best regards,\n"
    ...
    body_admin = (
        f"New equipment booking received at CancerLab.\n\n"
        f"━━━━━━ Booking Details ━━━━━━\n"
        f"  Booking ID   : #{bid}\n"
        f"  Equipment    : {equip}\n"
        f"  Date         : {date_fmt}\n"
        ...
        f"  FUB ID       : {fub}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Log in to the CancerLab admin panel to manage bookings."
    )

    # Always attempt BOTH recipients independently
    ok_user, err_user = _send_email_safe(
        email,
        f"[CancerLab] Booking Confirmed – {equip} on {date_fmt}",
        body_user,
    )
    ok_admin, err_admin = _send_email_safe(
        ADMIN_EMAIL,
        f"[CancerLab] New Booking #{bid} – {equip} on {date_fmt}",
        body_admin,
    )

    _email_logger.info(
        "send_booking_emails result | booking_id=%s | user_ok=%s | admin_ok=%s",
        bid, ok_user, ok_admin,
    )
...
def page_schedule_equipment():
    ...
    .sch-page { max-width:700px; margin:0 auto; padding:28px 20px 60px; }
    .sch-hero {
    ...
    .sch-hero h1 {
        ...
        color:var(--sch-ink); margin:0 0 6px;
    }
    .sch-hero p {
        color:var(--sch-muted); font-size:15px; margin:0;
    }
    .sch-card {
    ...
    .sch-tab {
        ...
        cursor:pointer; border:1px solid var(--sch-line);
        background:#fff; color:var(--sch-muted);
        transition:.15s;
    }
    .sch-tab.active {
        background:var(--sch-blue); color:#fff;
        border-color:var(--sch-blue);
        box-shadow:0 4px 12px rgba(42,77,181,.25);
    }
    .sch-badge-confirmed {
        display:inline-block; padding:2px 10px; border-radius:999px;
        background:var(--sch-green-light); color:var(--sch-green);
        font-size:12px; font-weight:800;
    }
    .sch-badge-cancelled {
        display:inline-block; padding:2px 10px; border-radius:999px;
        background:var(--sch-red-light); color:var(--sch-red);
        font-size:12px; font-weight:800;
    }
    .sch-booking-row {
    ...
    .sch-eq-pill {
        ...
        border-radius:999px; border:1px solid var(--sch-line);
        background:#f8faff; font-size:13px; color:var(--sch-ink);
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    # ======== DASHBOARD VIEW ========
        ...
        if st.session_state.sch_dash_tab == "new":
            st.markdown('<div class="sch-card">', unsafe_allow_html=True)
            st.markdown('<div class="sch-card-title">📅 Schedule Equipment</div>', unsafe_allow_html=True)
            ...
            with st.form("sch_booking_form"):
                ...
                # Time slots 08:00–14:00 in 30-min increments (last slot: 14:00)
                time_slots = []
                for h in range(8, 14):
                    for m in (0, 30):
                        time_slots.append(f"{h:02d}:{m:02d}")
                time_slots.append("14:00")  # include the 14:00 boundary slot
                ...
                sub_b = st.form_submit_button("Confirm Booking", use_container_width=True)

            if sub_b:
                date_str = str(date_sel)

                # --- Backend time validation (prevents manual bypass) ---
                try:
                ...
                if not _time_ok:
                    ...
                    try:
                        ...
                        if email_result.get("user_sent"):
                            st.success(f"✅ Booking confirmed! ID: #{bid}. A confirmation email has been sent to {u['email']}.")
                        else:
                            st.success(f"✅ Booking confirmed! ID: #{bid}.")
                            st.warning(
                                f"⚠️ Confirmation email could not be delivered to {u['email']}. "
                                f"Your booking is saved. Please contact the lab if needed. "
                                f"(Error: {email_result.get('user_error', 'unknown')})"
                            )
                        st.session_state.sch_dash_tab = "history"
                        st.rerun()
                    except Exception as ex:
                        st.error(f"Error saving booking: {ex}")
        ...
        # --- MY BOOKINGS TAB ---
            ...
            st.markdown('<div class="sch-card">', unsafe_allow_html=True)
            st.markdown(f'<div class="sch-card-title">📋 My Bookings <span style="font-weight:400;font-size:14px;color:#475569;">({len(my_bookings)} total)</span></div>', unsafe_allow_html=True)

            if not my_bookings:
                st.info("You have no bookings yet. Use the 'New Booking' tab to schedule equipment.")
            else:
                for bk in my_bookings:
                    badge = '<span class="sch-badge-confirmed">Confirmed</span>' if bk["status"] == "confirmed" else '<span class="sch-badge-cancelled">Cancelled</span>'
...
def render_manage_bookings():
    """Área interna: visualizar e gerenciar todos os agendamentos de equipamentos."""
    st.markdown("""
    <style>
    .mb-card {
    ...
    .mb-title { font-weight:900; font-size:17px; color:#0f172a; margin:0 0 14px; }
    .mb-row { border:1px solid #e2e8f0; border-radius:10px; padding:12px 14px; margin-bottom:8px; background:#fafbfe; }
    ...
    .mb-badge-ok { display:inline-block; padding:2px 9px; border-radius:999px; background:#e6f5ee; color:#0d7c4c; font-size:12px; font-weight:800; }
    .mb-badge-can { display:inline-block; padding:2px 9px; border-radius:999px; background:#fee2e2; color:#b91c1c; font-size:12px; font-weight:800; }
    </style>
    """, unsafe_allow_html=True)
    ...
    with col_st:
        st_filter = st.selectbox(
            "Filtrar por Status",
            ["Todos", "Confirmado", "Cancelado"],
            key="mb_st_filter",
        )
    # Normalize display label back to DB value
    _status_display_map = {"Confirmado": "confirmed", "Cancelado": "cancelled"}
    with col_dt:
        date_filter = st.date_input("Filtrar por Data (opcional)", value=None, key="mb_date_filter")
    ...
    if st_filter != "Todos":
        _status_map = _status_display_map
        filtered = [b for b in filtered if b["status"] == _status_map.get(st_filter, st_filter)]
    if date_filter:
    ...
    if not filtered:
        ...
        for bk in filtered:
            ...
            _status_pt = "Confirmado" if bk["status"] == "confirmed" else "Cancelado"
            badge_html = (
                f'<span class="mb-badge-ok">{_status_pt}</span>'
                if bk["status"] == "confirmed"
                else f'<span class="mb-badge-can">{_status_pt}</span>'
            )
            notes_txt = bk.get("notes") or "—"
            st.markdown(f"""
            <div class="mb-row">
                ...
                <div class="mb-user">
                    ...
                    👤 {html.escape(bk['user_name'])} — {html.escape(bk['user_email'])} — {html.escape(bk['user_phone'])} — Matrícula FUB: {html.escape(bk['user_fub'])}
                </div>
            ...
            if bk["status"] == "confirmed":
                if st.button(f"🚫 Cancelar Agendamento #{bk['id']}", key=f"mb_cancel_{bk['id']}"):
                    cancel_booking(bk["id"])
                    st.success(f"Agendamento #{bk['id']} foi cancelado com sucesso.")
                    st.rerun()
